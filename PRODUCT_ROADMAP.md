...
**一句话定位**：一款面向律师与法务团队的"安心智能助手"，以对话为核心入口，覆盖合同审查、合规自检、案件管理、知识检索、证据梳理与律师协作的全流程。
...
- 我的设置只处理“我如何使用”：个人资料、本机运行和通知偏好；不再承载组织级模型或 MCP 凭据 CRUD。
- 治理后台只处理“组织/平台如何治理”：用户角色、组织、安全审计、系统集成、模型治理、MCP/工具连接、计费和发布运维。
...
- [~] **P1-1**：本地 LLM 模型管理界面 — 本机运行已补默认本地模型选择、Ollama/兼容端点模型清单、secret-free 持久化、Quick Query 默认模型读取和本地模型端点本机/私网 guard；一键下载、真实模型 smoke 和 packaged runtime 证据待补
