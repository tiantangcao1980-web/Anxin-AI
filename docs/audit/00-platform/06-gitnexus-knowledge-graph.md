...
- 2026-05-06 当前工作树结构索引恢复：
  ...
  - 2026-05-08 10:13 使用同一 direct binary 与 `--clean-first` 完成过一次全量 embedding 写入；该运行通过 meta/Cypher 非零一致性、`gitnexus status` commit 一致性和 `detect-changes` clean check 验证。精确统计属于 commit-scoped 运行产物，后续以最新 `.gitnexus/meta.json` 与 Cypher count 为准，避免文档提交后硬编码统计过期。
- 当前结论：`.gitnexus/meta.json` 仍有非零 embedding 数据，但读取型 CLI（直接 rc binary）当前无法解析本仓 repo；同时 `.gitnexus` 落后于最新本地提交，不能作为最新提交放行证据。semantic query 不能单独作为需求覆盖或风险放行依据；后续新增提交仍需结合 `git status --short`、`rg`、直接源码阅读和专项 pytest/Vitest/Playwright 补齐事实。
...
| `npx gitnexus@rc` wrapper 可能触发 npm/arborist rebuild bug | 通过 npx 调 rc 的读取命令可能失败；当前 direct rc binary 也无法解析本仓 repo，需要最终复跑修复 | 使用 `GITNEXUS_BIN=/Users/pengchengkeji/.npm/_npx/ce85571ede75641e/node_modules/.bin/gitnexus` 重建并复验；semantic/context 结果仍需用 `rg`、源码阅读和测试补盲 |
...
| 当前索引 meta 可读但 CLI 复验失败且落后于最新提交 | embedding rows 已进入 `.gitnexus/meta.json`；direct rc binary 当前无法解析本仓 repo，最新提交必须重建后才能用于放行 | 以 `.gitnexus/meta.json`、最终 direct rc CLI 复验、源码阅读和测试作为证据组合；对 P0 仍以 `rg` + 直接源码阅读 + pytest/Vitest/Playwright 作为事实源 |
