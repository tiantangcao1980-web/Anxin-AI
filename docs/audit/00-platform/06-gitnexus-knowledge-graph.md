...
| embeddings rows | 以 `.gitnexus/meta.json` 与 `gitnexus cypher` 计数为准，二者必须非零且一致 |
| distinct embedded nodes | not separately verified |

`GITNEXUS_BIN=/Users/pengchengkeji/.npm/_npx/ce85571ede75641e/node_modules/.bin/gitnexus bash scripts/gitnexus-index.sh --embeddings --clean-first --skip-context-checks` 已在本轮本地交付提交后完成全量重建。当前 `.gitnexus/meta.json` 必须显示 `capabilities.vectorSearch.status=vector-index`，embedding stats 和 store 均可读。验证标准：`cypher MATCH (e:CodeEmbedding) RETURN count(e) AS cnt` 返回非零计数并与 meta embeddings 一致，`gitnexus status` 显示 indexed/current commit 一致且 `Status: up-to-date`，`detect-changes --repo Anxin-Smart-Legal-Services` 返回 `No changes detected`。注意：`npx gitnexus@rc` 在本机仍可能触发 npm/arborist rebuild bug；推荐使用 `GITNEXUS_BIN=/Users/pengchengkeji/.npm/_npx/ce85571ede75641e/node_modules/.bin/gitnexus`。
...
- 2026-05-06 当前工作树结构索引恢复：
  ...
  - 2026-05-08 10:13 在本轮本地交付提交后使用同一 direct binary 与 `--clean-first` 重新完成全量 embedding 写入；该运行通过 meta/Cypher 非零一致性、`gitnexus status` commit 一致性和 `detect-changes` clean check 验证。精确统计属于 commit-scoped 运行产物，后续以最新 `.gitnexus/meta.json` 与 Cypher count 为准，避免文档提交后硬编码统计过期。
