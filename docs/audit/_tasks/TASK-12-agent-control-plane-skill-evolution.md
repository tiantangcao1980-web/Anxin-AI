...
| P0-3 | 真实密钥隔离 | 持久化 route-token lease 已只存 hash，跨服务实例可验证；MCP tool execution、CLI `/execute`、REST/SSE/WebSocket Chat Agent LLM runtime、RAG direct LLM 与浏览器/爬虫 fetch 入口已在商业环境接入 DB-backed route token；desktop-control 后端命令队列已收紧为 safe-probe-only，非 safe-probe 命令在入队前拒绝并审计；完整浏览器自动化和 desktop-control 高风险真实调用链尚未全部改造 | Agent/Worker 只拿短期 consumer token 或等价 route token；真实 API key/PAT/财税凭据只在密钥服务/网关侧 |
...
- `backend/src/api/routes/agent_approvals.py` 已补正式 API：创建审批、按组织/本人 scope 列表与详情、pending count、audit-events、audit-export、approve/reject/revoke、workspace-control observe、默认 pause/takeover/terminate fail-closed 和显式 local runtime rehearsal、执行前 validate、workspace artifacts list/create/revise/export，并由 `backend/tests/test_agent_approval_api.py` 覆盖未登录拒绝、普通员工不能自批、管理员批准后 validate 放行、撤销后 fail-closed、审批审计时间线/导出、observe 只读旁听快照、运行时未接入时暂停/接管/终止拒绝、本地 rehearsal 接管成功、已批准工作室成果记录/复核/导出和 payload/artifact 脱敏。
- `frontend/src/pages/AgentApprovalWorkspace.tsx` 已补最小 Human-in-the-loop 高风险审批入口：按状态筛选、展示风险动作/route/payload 预览、批准/驳回/撤销、审批审计时间线、JSON 导出、已批准工作室控制入口和成果复核入口，并接入 `/agent-approvals` API、桌面侧边栏、移动协作导航；能力策略面板已按当前组织角色过滤可见工具，老板/Owner/超级管理员/admin 类角色看全量，普通员工只看基础能力和可申请能力，高风险/full-only 工具保持隐藏；`frontend/e2e/agent-approval-workspace.spec.ts` 覆盖桌面审计/导出/批准动作、成果复核、运行时未接入时控制 fail-closed、员工能力中心可见边界和移动端视口不横向溢出。
...
- 已批准工作室的最小 artifact-first 成果流已补：管理员可记录脱敏 summary artifact、列表查看、复核修订和 JSON 导出，前端工作台可查看/新增/复核/导出工作室成果，并可触发 observe 获取只读旁听快照和审计事件；桌面聊天工作台也可接收 runtime-generated long-task summary artifact event 并缓存/展示。仍缺真实暂停/接管/终止执行效果、签名/商业 runtime 长任务 replay、跨端恢复和真实执行链路接入。
...
- [x] AgentApproval service/API 和最小前端工作台具备创建、列表/count、审批/驳回/撤销、过期、action/route 匹配、执行前 validate、组织/本人 scope、审批审计时间线、审批审计 JSON 导出、已批准工作室 artifact list/create/revise/export、observe 只读旁听快照、运行时未接入时 pause/takeover/terminate workspace-control fail-closed、显式 local runtime rehearsal、移动视口回归和审计回归；完整工作室和真实执行链路接入仍未完成。
...
- [x] 高风险工作室已具备最小 artifact-first 成果记录、查看、复核修订和导出代码级闭环；桌面聊天工作台已具备长任务 runtime summary artifact 事件展示/缓存代码级闭环。
- [ ] 高风险工作室支持完整运行时旁听、真实暂停/接管/终止执行效果、签名/商业 runtime 长任务 replay、跨端恢复和真实运行时证据。
