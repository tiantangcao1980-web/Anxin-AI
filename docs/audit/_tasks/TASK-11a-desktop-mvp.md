...
2026-05-10 macOS Keychain 弹窗收敛记录：`desktop/src/services/secure_db.rs` 新增进程内 `SQLCIPHER_KEY_CACHE`，首次成功读取/生成 SQLCipher key 后复用该 key，避免每个业务实体查询或每次 SQLCipher 连接都重新触发系统 Keychain 授权；显式 `secure_db_reset_local_data` / smoke key 删除会同步清理缓存。`desktop/src/services/runtime_config.rs` 加入 `database_file_exists` guard，本地 DB 不存在时不为读取可选 `app_settings` 镜像创建 Keychain 访问。`docs/desktop/sqlite-encryption-strategy.md` 和 `docs/DEPLOYMENT_DESKTOP.md` 已写明：弹窗是 macOS 对单一 SQLCipher DB 主密钥的授权，不是每个实体都要用户输入密码；debug/unsigned 二进制因签名身份不稳定仍可能首次弹窗，商业放行仍需 signed/notarized runtime 证据。
