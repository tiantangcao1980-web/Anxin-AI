...
- [x] 经验沉淀到 hierarchical-memory（add-feature 至少 1 条，记录三件套的 Tauri 平台分支模式）
...
2026-05-08 远控 host 补证：`desktop/src/services/remote_control_host.rs` 与 `desktop/src/commands/remote_control.rs` 已在单次 safe-probe host cycle 基础上新增 bounded safe-probe host poll：轮询间隔、最大轮询次数和每次 claim limit 都有上限，且仍只完成 `ping/status_probe`，未知/高风险命令继续 failed/unsupported。该轮 `cd desktop && cargo test remote_control` 为 `8 passed`，`cd desktop && cargo check` 通过。此记录只降低 host 侧轮询缺口，不关闭完整 DoD；daemon 基础在 2026-05-09 记录继续推进，桌面确认 UI、真实跨设备 host callback、signed runtime 和真机/设备证据仍需补。

2026-05-09 远控 host daemon 基础补证：`desktop/src/services/remote_control_host.rs` 新增 env-gated 后台 safe-probe daemon 配置与运行函数，`desktop/src/lib.rs` 在 Tauri setup 中按开关启动；未设置开关时不启动，缺 `desktop_device_id` / `pairing_id` / `route_token` / `host_instance_id` 时拒绝，绝密模式和未登录/无本地测试 bearer 时均在网络前阻断，且仍复用 bounded poll 与 safe-probe-only 决策。`cd desktop && cargo test remote_control` 该轮 `13 passed`，`cd desktop && cargo check` 通过。此记录补齐的是可执行 daemon 基础，不等于 signed runtime 常驻运行、真实跨设备 host callback、高风险执行器或设备证据完成。

2026-05-09 远控 host daemon 本地回路补证：`desktop/src/services/remote_control_host.rs` 新增本地 mock 后端测试，证明后台 safe-probe daemon 能完成一次 claim -> running -> completed 状态回传，并断言 completed `result_summary` 不包含 route token 或 bearer token。`cd desktop && cargo test remote_control` 当前 `14 passed`。此记录补齐的是本地函数级/流程级回路，不等于 signed runtime 常驻运行、真实跨设备 host callback、高风险执行器或设备证据完成。
...
2026-05-09 远控 host 可见控制面补证：`frontend/src/components/desktop/DesktopWorkstationPanel.tsx` 新增“移动远控 host”面板，在桌面运行时显示配对状态、权限控制项、待领取命令、最近审计时间线和最近 safe-probe host cycle 汇总；待桌面确认时可通过 Tauri IPC `remote_control_confirm_pairing` 确认配对，已确认配对时先经前端 API 申请短期 `desktop:control` route token，再调用 Tauri IPC `remote_control_run_host_cycle` 只执行 safe-probe host cycle；queued/claimed 命令可从脱敏审计事件中定位并调用后端 cancel API 取消。`frontend/src/components/desktop/desktopWorkstationModel.ts` 新增 host UI 状态模型，锁住绝密/非桌面不可操作、待确认只允许确认、已确认只允许安全探针、终态命令不可取消；`frontend/src/lib/api.ts` 和 `frontend/src/lib/tauri-bridge.ts` 补最小 API/IPC wrapper；`frontend/e2e/settings-workstation.spec.ts` 覆盖 pending pairing 确认、ready pairing safe-probe/cancel 控制、非桌面按钮 disabled 和移动宽度。验证：`cd frontend && npm test -- desktopWorkstationModel.test.ts` 为 `9 passed`，`npm exec tsc -- --noEmit` 通过，`npm run lint` 通过，`npx playwright test e2e/settings-workstation.spec.ts --project=chromium` 为 `7 passed, 1 skipped`，`--project=mobile` 为 `8 passed`。此项补齐的是桌面 host 可见控制面和 mock runtime e2e，不等于 signed runtime daemon 常驻运行、真实跨设备 host callback、高风险执行器或真机证据完成。
...
2026-05-10 macOS Keychain 弹窗收敛记录：`desktop/src/services/secure_db.rs` 新增进程内 `SQLCIPHER_KEY_CACHE`，首次成功读取/生成 SQLCipher key 后复用该 key，避免每个业务实体查询或每次 SQLCipher 连接都重新触发系统 Keychain 授权；显式 `secure_db_reset_local_data` / smoke key 删除会同步清理缓存。`desktop/src/services/runtime_config.rs` 加入 `database_file_exists` guard，本地 DB 不存在时不为读取可选 `app_settings` 镜像创建 Keychain 访问。`docs/desktop/sqlite-encryption-strategy.md` 和 `docs/deployment-desktop.md` 已写明：弹窗是 macOS 对单一 SQLCipher DB 主密钥的授权，不是每个实体都要用户输入密码；debug/unsigned 二进制因签名身份不稳定仍可能首次弹窗，商业放行仍需 signed/notarized runtime 证据。
...
2026-05-09 hierarchical-memory 沉淀记录：已通过 `python3 ~/.claude/skills/hierarchical-memory/scripts/memory-cli.py add-feature` 写入 `tauri-desktop-mvp-trio`，记录 macOS Overlay / Windows-Linux decoration 分支、Quick Query 独立窗口、文件拖入 SQLCipher 离线队列、桌面工作站配置/远控 safe-probe 控制面和本地结构门禁模式。证据 ID：`feature-1778335460725`；`find-feature "tauri desktop mvp trio quick query file drop workstation"` 可检索到该条记录。
