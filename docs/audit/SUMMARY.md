...
| 创建并优化 GitNexus 索引知识图、配置 embeddings | `.gitnexusignore` 已生效；当前 `.gitnexus/meta.json` 为约 `1.3k files / 32.5k nodes / 58.8k edges / 300 flows / 30.4k embeddings`，`capabilities.vectorSearch.status=vector-index`，精确统计以最新 meta 为准；`scripts/gitnexus-index.sh` 支持 `GITNEXUS_BIN`、`GITNEXUS_NPX_SPEC=gitnexus@rc` 和 embedding/vector-index preflight；direct rc binary 的 `cypher/status/detect-changes` 已验证，current/indexed commit 一致 | embeddings 已生成并可读 |
