...
> 结论：当前不是商业交付完成态；已完成 GitNexus 可复跑脚本、OpenSpec/验收规范、多项 P0 代码级收口、内建 RAG full50、桌面 release package dry-run/preflight、unsigned release packaged sync loopback smoke 和完成度审计。上一轮 clean baseline 的 `.gitnexus/meta.json` 已索引到 commit `ccb316d8`，包含 `34002` embeddings；本轮后续提交仍必须刷新到最新 `HEAD` 后才能作为发布放行证据。剩余阻断集中在真实渠道沙箱、桌面签名/公证发布包、移动/小程序真机、案件/市场等未闭环模块和发布前全量门禁。

## 1. 目标到证据清单

| 用户目标/显式要求 | 当前证据 | 状态 |
|---|---|---|
| 使用 CLI/MCP 对代码仓库做代码级扫描 | `.gitnexus/meta.json` 上一轮 clean baseline 已重建到 commit `ccb316d8`，统计为 files `1431`、nodes `36114`、edges `65135`、processes `300`、embeddings `34002`；本轮若形成新提交，仍需再次刷新，事实以 `rg` + 源码阅读 + 测试互证为准 | GitNexus 可复跑入口已建立；每次最终提交后必须复验 GitNexus |
| 创建并优化 GitNexus 索引知识图、配置 embeddings | `.gitnexusignore` 已生效；`scripts/gitnexus-index.sh` 支持 `GITNEXUS_BIN`、`GITNEXUS_NPX_SPEC=gitnexus@rc` 和 embedding/vector-index preflight；最终 release 前仍要求 `.gitnexus/meta.json` 与 `gitnexus cypher/status/detect-changes` 对当前 commit 非零一致 | 上一轮 clean baseline 可用；本轮提交后需刷新 |
| 在后续开发前验证 GitNexus 信息准确性 | 当前策略保持 `rg` + 源码阅读 + pytest/Vitest 为事实源；GitNexus direct CLI 在提交后刷新通过时作为辅助导航与 release evidence | 已建立边界；最新提交前不跳过刷新 |
...
| 发布门禁 | `scripts/static-quality-baseline.sh` 已采集当前静态质量基线：ruff `0`、mypy `0`、mypy zero-baseline gate `0`、release evidence secret/PII scan `0`、secret scan `0`、mock/fallback scan `0`；全仓 Ruff 与 backend mypy 已清零；上一轮 GitNexus 已刷新到 `ccb316d8`，但真机/沙箱/e2e 覆盖不足，且每次新提交后 GitNexus 必须重建到最新 `HEAD` | 静态质量门槛已闭合，发布前仍不应宣称 full commercial ready |
...
5. 每次最终提交后重跑 `GITNEXUS_BIN` direct rc CLI 并确认 `lastCommit == HEAD`；日常开发继续以 `rg`、源码阅读和测试为主。
