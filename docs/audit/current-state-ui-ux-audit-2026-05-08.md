...
| 桌面 UX | 同步冲突应给非技术用户可理解的决策界面 | `frontend/src/components/mode-switcher/SyncStatus.tsx:176` 直接展示 JSON；按钮尺寸偏小；tray 使用 emoji 标签 `desktop/src/services/tray.rs:13` | 政务用户难理解，桌面专业感不足 | 冲突摘要化、字段差异化、按钮尺寸和菜单文案按桌面规范修正 |
