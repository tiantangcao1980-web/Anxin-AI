...
- Desktop native notification has code-level coverage: `desktop/src/commands/native_notification.rs` sanitizes local payloads, exposes permission status/request commands, `frontend/src/lib/tauri-bridge.ts` exposes typed IPC wrappers, and the workstation panel can request permission and send a local-only test notification without external push infrastructure.
