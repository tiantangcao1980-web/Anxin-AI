...
- 外部资源交接已机器化：`docs/release/external-resource-requirements.json` 结构化列出支付、电签、桌面签名/公证、uni-app/DCloud/iOS/Android/微信小程序、LLM/Embedding/connector 等外部输入；`scripts/validate-external-resource-requirements.cjs` 与 commercial gate 会检查 P0 env 模板同步、handoff 文档存在和禁止真实凭据值入仓。
