...
> 日期：2026-05-06
> 状态：backend durable log implemented; Rust-owned SQLCipher local DB path and code-level retry/backoff implemented; packaged UI/runtime evidence pending.
...
- Desktop sync engine: `frontend/src/lib/api-adapter.ts` reads pending local changes through Rust Tauri SQLCipher commands, pushes to `/api/v1/sync/push`, pulls from `/api/v1/sync/pull`, writes local tables, updates `sync.last_server_version`.
...
- Real packaged Tauri smoke for migration application, push/pull/conflict/retry, and merge editing.
