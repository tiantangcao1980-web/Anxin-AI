...
| 移动/小程序本地门禁 | `bash scripts/mobile-device-smoke.sh` | 通过：mobile Vitest 6 files / 15 tests passed；mobile tsc、小程序 tsc/build、fake fallback guard、mini-program design token guard 均为 exit 0 |
...
```text
430 passed, 1 skipped, 10 warnings
```

但注意：当前工作区有大量未提交改动。发布前必须重新跑全量：
...
该脚本覆盖移动 Vitest、移动 tsc、小程序 tsc/build、fake fallback guard 与小程序设计 token guard。它不能替代 iOS、Android、微信开发者工具或真机 transcript。

## GitNexus 风险信号

GitNexus `detect_changes(scope=all)` 曾报告：
