...
- 审批流已补本地授权防线：普通成员不能批量审批他人/跨组织审批；审批列表按用户/组织 scope 过滤；`admin` 按 `org_admin` 语义只管本组织，只有 `super_admin` 可全局；模板写入仅限 `admin/org_admin/super_admin` 且模板读取/使用按创建者或全局管理员过滤；过期审批不能 approve/reject/batch 继续执行。AgentApprovalService 和 `/agent-approvals` API 已补高风险智能体审批的创建、组织/本人 scope、payload 脱敏、授权角色审批/驳回、撤销、执行前 validate、过期/撤销 fail-closed、action/route 匹配和审计写入。商业发布前仍需补正式 Agent Approval 工作室 UI、审批审计导出和组织级模板 schema。
