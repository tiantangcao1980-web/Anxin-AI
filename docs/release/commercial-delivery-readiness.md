...
| 全设备智能助手定位 | 后端已有 LLM、private LLM、MCP、Skills、knowledge 代码底座；CLI Key 已绑定登录用户与最小 scope，并写入 key/execute 审计；MCP 管理 API 已收紧到 `manage:system`，管理动作写入脱敏审计，独立 MCP 商业环境默认关闭；agent capability policy 已补未知工具 fail-closed、订阅/权限/隐私/设备/通道/审批上下文和 MCP tool runtime 二次判权；桌面已有 local LLM 命令、SQLCipher/keyring 和本地队列基础；移动已有隐私模式和本地模型语义 | 桌面主工作站配置面、任意 LLM/Skills/MCP 可配置验收、MCP stdio/url/env allowlist、独立本地知识库体验、移动远程控制桌面、绝密模式出站 fail-closed 全链路证据仍缺 |
...
- 桌面主工作站还需补本地模型、独立知识库、Skills/MCP、隐私模式状态和执行审计的统一配置入口；当前 CLI/MCP 与 agent capability policy 已先做最小权限/fail-closed 加固，但还缺组织级 capability route、短期 consumer token、MCP stdio/url/env allowlist、调用审计与撤销后立即失权；移动远控桌面还需补设备配对、命令队列、状态回传、取消/撤销、敏感动作二次确认和审计证据。
