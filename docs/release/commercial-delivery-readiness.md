...
- 桌面主工作站还需补本地模型、独立知识库、Skills/MCP、隐私模式状态和执行审计的统一配置入口；当前 CLI/MCP、外部 MCP allowlist 与 agent capability policy 已先做最小权限/fail-closed 加固，但还缺组织级 capability route、短期 consumer token、真实 approved MCP connector 演练、调用审计与撤销后立即失权；移动远控桌面还需补设备配对、命令队列、状态回传、取消/撤销、敏感动作二次确认和审计证据。
