...
- 2026-05-09 已补桌面离线队列本机管理与文本摘要处理：`list_offline_tasks` 暴露最近任务脱敏摘要，`retry_failed_offline_tasks` 只把 failed 任务重新放回本机 SQLCipher 队列，不触发网络同步；`process_local_offline_tasks` 当前只处理桌面文件拖入的 `.txt/.md` `document_summary`，以内置规则摘要写入 `local_result` 并推进到 `local_completed`，不调用外部模型、云端 API 或同步；本机运行 UI 可查看队列统计、最近任务、失败重入队和本机处理，且桌面文件拖入任务不会暴露完整本地路径。该证据仍不替代 PDF/DOCX 本机解析、shared-staging 同步、跨设备连续会话、signed packaged runtime 或完整脱网业务闭环。
