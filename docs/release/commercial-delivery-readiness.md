...
- 2026-05-09 已补桌面本机通知：`desktop/src/commands/native_notification.rs` 提供 `preview_desktop_notification` / `send_desktop_notification`，案件进展、风险预警、系统和同步状态通知通过 Tauri 本机通知插件发送，payload 会先做空白压缩、长度限制和关联 ID 控制字符校验；工作站 UI 新增“本机通知”状态和测试按钮，TopSecret 模式下仍声明 local-only / safe-in-top-secret，不接入 APNs、FCM 或服务端推送。该证据仍不替代 signed packaged runtime 通知权限、后台/托盘触发、真实案件事件驱动或跨设备推送证据。
