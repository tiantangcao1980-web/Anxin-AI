...
- 桌面主工作站统一入口已补最小可见状态：用户可在设置页看到隐私模式、本地模型、独立知识库、Skills/MCP、跨端同步和移动远控状态，且绝密模式不会展示会导致数据出站的可执行同步/远控动作；非桌面预览下，本地模型、同步和移动远控保持 disabled，避免把浏览器/手机误呈现为桌面控制台；当前 CLI/MCP、外部 MCP allowlist、桌面 IPC TopSecret guard、WebView/API data-network guard、Tauri/前端 HTTP 插件移除、CSP 收紧与 agent capability policy 已先做最小权限/fail-closed 加固，但还缺完整配置 CRUD、真实本地模型/知识库状态探针、组织级 capability route、短期 consumer token、真实 approved MCP connector 演练、signed packaged runtime 出站复验、调用审计与撤销后立即失权；移动远控桌面还需补设备配对、命令队列、状态回传、取消/撤销、敏感动作二次确认和审计证据。
