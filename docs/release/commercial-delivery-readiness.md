...
- 2026-05-09 已补桌面本地模型默认配置：`runtime-config.json` 增加 secret-free `localModel`，`local_llm_chat` 未显式传模型时读取桌面默认模型，工作站 UI 可查看本地端点/模型数量并保存默认模型；本地门禁已锁住 `get_local_llm_config`、`set_default_local_model`、模型名称安全字符集和 Playwright 保存流程。该证据仍不替代真实 Ollama 模型 smoke、一键下载/安装、packaged runtime 或 signed runtime 证据。
