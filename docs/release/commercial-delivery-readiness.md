...
| 移动/小程序真机 | `scripts/mobile-device-smoke.sh` 已通过本地门禁：移动端 `8 files / 37 tests`、tsc、mobile result surface guard、mobile lawyer conversion guard、desktop-control status endpoint/local-mode no-network regressions、Expo config/SDK/Metro config guard、小程序 tsc/build、mini-program privacy boundary guard、mini-program navigation boundary guard、WeChat DevTools CLI project smoke、refresh auth guard、mobile/mini privacy network guard、fake fallback guard、mini-program design token guard；最新移动端独立 Vitest 在 safe-probe status/cancel/audit timeline hardening 后为 `8 files / 37 tests passed`，typecheck 通过；`npx expo-doctor` `17/17 checks passed`；`scripts/mobile-ios-simulator-smoke.sh` 已生成 iPhone 17 Simulator + Expo Go 支持性 app-run 证据并完成 `iOS Bundled`，但 artifact 明确 `release_evidence_complete=false`；messages/tasks 详情页已移除静默假数据兜底；小程序首页/个人中心主入口不再有空路径或“功能开发中”死胡同；尽调与知识库搜索入口已改为真实 API + 加载/错误态 + 页面内结果摘要卡；移动端找律师已具备匿名咨询转化链路，不再只是律师列表；移动端 `/desktop-control` local 模式不发网络请求，confirmed pairing 下可先申请短期 route token 再只入队 `desktop.status_probe`，并支持刷新 queued 命令状态、取消 queued/claimed 探针和查看脱敏审计时间线，且仍不伪造桌面执行成功；移动/小程序 refresh-token 已避免弱网误清会话；移动 production `npm audit --omit=dev` 已清零并写入 `mobile_npm_audit.status=passed,total=0`；小程序 local/top-secret 会在数据请求与登录前 fail-closed；缺真实/官方 iOS 设备故事、Android、交互式微信开发者工具、跨设备连续会话和完整真机验收 | 阻断 |
| 全设备智能助手定位 | 后端已有 LLM、private LLM、MCP、Skills、knowledge 代码底座；CLI Key 已绑定登录用户与最小 scope，并写入 key/execute 审计，CLI route-token 签发与桌面端 execute 前获取/传递已补；MCP 管理 API 已收紧到 `manage:system`，管理动作写入脱敏审计，独立 MCP 商业环境默认关闭；MCP 外部连接已补 stdio 默认关闭、command/command-line/env allowlist、SSE scheme/host allowlist 和子进程最小环境；agent capability policy 已补未知工具 fail-closed、订阅/权限/隐私/设备/通道/审批上下文、服务层单次可审计统一能力决策和 MCP tool runtime 二次判权；桌面已有 local LLM 命令、SQLCipher/keyring、本地队列，以及 TopSecret CLI/sync/Harness IPC、本地同步桥、统一 API fetch、WebView 全局 fetch/WebSocket 出站 guard；Tauri 原生 HTTP 插件、前端 HTTP 插件包、活跃 `http:*` capability、宽泛 shell/opener launch capability 和宽松 CSP 已由 `scripts/desktop-network-surface-gate.sh` 收紧；移动已有隐私模式和本地模型语义；`/settings?tab=workstation` 已补桌面主工作站最小入口、本地 LLM/模型数量/离线队列、知识库/MCP 只读探针和移动远控安全闸，并用模型测试锁住绝密模式下同步/远控不启用、知识库/MCP 数据网络探针跳过、非桌面预览不启用本地模型/同步/远控动作；后端 `/sync/remote-control/*` 已补 DB-backed 移动远控控制面和 host lifecycle API，锁住本地/绝密模式、桌面确认、CapabilityRoute route-token、命令队列、host 领取、running/completed/failed 状态回传、取消和递归脱敏审计；桌面 Tauri IPC host 客户端已能按登录态 bearer token 调用 confirm/claim/status API，且单次 host cycle 只完成 safe probe、未知命令 failed/unsupported，并新增 bounded safe-probe poll 命令，由 TopSecret guard 与 payload/状态/轮询配置单测锁住；移动端 `/desktop-control` 已接入该安全闸，confirmed pairing 下只允许申请短期 route token 并入队 `desktop.status_probe`，并可刷新/取消未终态探针、查看脱敏审计时间线，不提供假执行成功；Playwright 已覆盖 direct/legacy route、tab URL 写回、非桌面禁用态、模拟桌面运行时探针和移动宽度不越界 | 完整桌面主工作站配置面、任意 LLM/Skills/MCP 可配置验收、真实 approved MCP connector 演练、真正常驻后台 daemon 与高风险真实执行器、真实移动远控跨设备联调、绝密模式 signed packaged runtime 出站复验证据仍缺 |
| 可信会话与 Skills 进化 | OpenSpec 已纳入 Codex/Claude 式过程可见、artifact-first、可打断/恢复和能力可发现体验；本地 `SkillEvolutionService` 已覆盖 proposal、required eval、授权审批、灰度、回滚、审计和 governed `SkillService` 版本过滤；DB-backed `SkillGovernanceService` 与 `/skill-governance` API 已覆盖 proposal、required eval、授权审批、灰度启用、回滚、enabled version 持久化、组织隔离、API scope、audit-events、audit-export、append-only audit 和 migration upgrade/downgrade；AgentApproval/AgentAuditEvent 模型、AgentApprovalService 和 `/agent-approvals` API 已补，覆盖高风险审批创建、组织/本人 scope、授权角色审批/驳回、撤销、执行前 validate、过期/撤销 fail-closed、action/route 匹配、payload 脱敏、审计写入、scoped audit-events API、audit-export API、workspace-control fail-closed、workspace artifact list/create/export，以及组织 CapabilityRoute list/update、policy 脱敏和禁用撤销 lease；前端最小 `Agent 治理工作台` 已支持审批列表/count、状态筛选、风险 payload 预览、批准/驳回/撤销、审批审计时间线、审批审计 JSON 导出、运行时未接入时控制动作拒绝提示、按组织角色过滤的能力策略可用/可申请/隐藏工具可视化、组织 route 启用/禁用、工作室成果查看/新增/导出、Skill 提案创建、评测、批准、灰度、审计时间线、审计 JSON 导出、桌面侧边栏入口和移动协作导航高亮，并有 Playwright 桌面/移动回归 | 长任务时间线、runtime 产物生成、artifact 编辑、跨设备恢复、记忆治理、完整 Human-in-the-loop 旁听、真实暂停/接管/终止执行效果、真实执行链路失权和商业发布证据仍缺 |
...
- 桌面主工作站统一入口已补最小可见状态：用户可在设置页看到隐私模式、本地模型、独立知识库、Skills/MCP、跨端同步和移动远控状态，且绝密模式不会展示会导致数据出站的可执行同步/远控动作；非桌面预览下，本地模型、同步和移动远控保持 disabled，避免把浏览器/手机误呈现为桌面控制台；桌面运行时下已通过现有 Tauri IPC 和受治理前端 API 只读展示本地 LLM 可用性、模型数量、离线队列、知识库数量/文档数和 MCP 服务/启用/工具缓存统计，绝密模式会跳过知识库/MCP 数据网络探针，并把移动远控保持在安全闸/待验收状态；浏览器级 e2e 已覆盖 workstation/legacy privacy route、URL 写回、模拟桌面运行时探针和 iPhone 14 视口资源卡不越界；后端移动远控 API 已补 DB-backed 配对、桌面确认、route-token、命令队列、host 领取、running/completed/failed 状态回传、取消和递归脱敏审计，桌面 Tauri IPC host 客户端已能确认配对、领取命令、回传状态、显式运行 safe-probe 单次循环，并新增有间隔/次数/limit 上限的 bounded safe-probe poll，移动端 `/desktop-control` 已把该安全闸推进到 safe-probe 入队、撤销与审计可见面：只在 confirmed pairing 同时具备 `desktop_device_id` 和 `pairing_id` 时申请短期 route token 并入队 `desktop.status_probe`，可刷新命令状态、取消 queued/claimed 探针并查看脱敏审计时间线，同时保留 local 零网络和“不声称已执行”的边界；当前 CLI/MCP、外部 MCP allowlist、MCP tool execution route-token、CLI `/execute` route-token、CLI route-token 签发与桌面端 execute 前获取/传递、REST/SSE/WebSocket Chat Agent LLM route-token、桌面 IPC TopSecret guard、WebView/API data-network guard、Tauri/前端 HTTP 插件移除、CSP 收紧、agent capability policy、服务层 unified capability policy engine、短期 route-token broker、Agent 控制面模型/迁移、DB-backed AgentGovernanceService、AgentApprovalService、DB-backed SkillGovernanceService、`/agent-approvals` API、`/skill-governance` API、Harness 能力策略 API 和最小 Agent 治理工作台已先做最小权限/fail-closed 加固，模型层已包含复合租户外键、TokenLease/SkillEnabledVersion 边界、hash-only token 和审计不可变防线，服务/API/UI 层已覆盖跨实例验证、组织隔离、consumer/scope 拒绝、consumer mismatch、过期、route 撤销后下一次调用失败、统一能力决策审计、按组织角色过滤的能力策略可用/可申请/隐藏工具展示、高风险审批组织/本人 scope、普通员工自批拒绝、管理员 approve/revoke、执行前 validate、过期/撤销/action/route mismatch 拒绝、payload 脱敏、审批审计时间线、审批审计 JSON 导出、workspace-control 未接入运行时前 fail-closed、Skill proposal/eval/approval/gray/rollback 持久化、Skill API scope、Skill 前端提案/评测/批准/灰度/审计导出、桌面控制拒绝提示、桌面批准动作和移动端不横向溢出；但还缺完整配置 CRUD、RAG direct LLM、完整 browser automation 和 desktop-control 运行时集成、真实 approved MCP connector 演练、signed packaged runtime 出站复验、工具调用细粒度审计和真实能力链路撤销后立即失权；移动远控桌面还需补真正常驻后台 daemon、高风险真实执行器、真实跨设备联调和真机证据。
- REST/SSE/WebSocket Chat Agent LLM runtime 已补 DB-backed `llm:chat` route-token gate，`/chat/route-token` 可签发用户绑定短期 token，WebSocket 可通过 auth 首包、连接 header 或单条消息 payload 传递 route-token，商业环境下 Agent 同步/流式模型调用会在真实 HTTP 前 fail-closed；当前证据是 `backend/tests/test_llm_route_governance.py` `8 passed`。浏览器/爬虫 fetch 入口也已补 DB-backed `browser:fetch` route-token gate：商业环境下授权发生在 robots/http 触网前，缺 token、consumer/scope 不匹配或 route 撤销都会 fail-closed；当前证据是 `backend/tests/test_crawler_compliance.py` `7 passed`，仍不替代 RAG direct LLM、完整 browser automation、desktop-control 和真实 connector runtime 证据。
- 可信 AI 交互还需补长任务过程可见、工具状态、证据引用、runtime 产物生成、artifact 编辑、真实暂停/恢复/接管/终止执行效果、命令面板和跨设备继续证据；Skills 进化已有本地和 DB-backed proposal/eval/approval/gray release/rollback/audit gate、版本过滤测试、最小前端治理面板、组织 route 启用/禁用面板、workspace artifact 面板和 workspace-control fail-closed 回归，但还需补完整策略编辑器、订阅/部门申请流、记忆治理、真实执行链路失权和商业发布证据。
...
- 移动端和小程序真机覆盖登录、审批、消息/任务详情、会话延续、错误态；本地 fake fallback guard 和 iOS Simulator Expo Go app-run 已过，但真实/官方 iOS、Android、交互式微信开发者工具、跨设备和桌面远控证据仍需补齐。
