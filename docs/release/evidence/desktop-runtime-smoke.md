...
| Push | Seed local `sync_log` pending row, run sync, verify backend receives row | frontend bridge code-level pass; Rust IPC fallback code-level pass; packaged-binary sync code smoke pass; packaged backend runtime pending | `frontend api-adapter.sync.test.ts`; `docs/release/evidence/artifacts/desktop-rust-ipc-sync-code-smoke-20260509.json`; `bash scripts/desktop-release-runtime-smoke.sh --app desktop/target/debug/bundle/macos/安心法务.app --ui-log-out /tmp/anxin-debug-release-runtime-sync-code-smoke.log` -> `release app sync code smoke` passed |
...
| desktop unsigned release packaged runtime smoke | exit `0`; release bundle binary self-test validates 8 local/sync tables plus SQLCipher/keyring security, packaged sync code smoke validates migration/pending/conflict/retry/pull decode, runtime startup exits cleanly, and WebView page-load handshake returns `pageLoadFinished=true`, `webviewLabel=main`, `url=tauri://localhost`; artifact has `signed_or_notarized=false`, `release_sync_code_smoke=passed`, and `release_evidence_complete=false` | `docs/release/evidence/artifacts/desktop-release-runtime-unsigned-smoke-20260509.json`; UI transcript `docs/release/evidence/artifacts/desktop-release-runtime-ui-smoke-unsigned-20260509.log` |
