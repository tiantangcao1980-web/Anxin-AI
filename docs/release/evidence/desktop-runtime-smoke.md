...
| Keyring | App restarts and reopens encrypted DB through platform key store | local installed-profile keyring reopen pass; unsigned release packaged-profile keyring reopen pass; runtime code now caches the SQLCipher key after the first key-store read and skips optional `app_settings` mirror probes when no DB exists; signed/notarized packaged evidence pending | release packaged-profile smoke and installed-profile smoke both run twice with an isolated real keyring service/user; reports have `keyringRoundTrip=true` and `encryptedReopen=true` on both launches; 2026-05-10 code path documents that macOS prompts are for one DB key, not per entity |
...
| desktop Tauri debug `.app` bundle build | exit `0`; built `desktop/target/debug/bundle/macos/安心智能助手.app` with `--debug --ci --bundles app --no-sign` |
...
| desktop debug `.app` packaged-binary sync code smoke | exit `0`; `scripts/desktop-release-runtime-smoke.sh --app desktop/target/debug/bundle/macos/安心智能助手.app --ui-log-out /tmp/anxin-debug-release-runtime-sync-code-smoke.log` calls the app binary `--sync-code-smoke` and verifies migration, pending/deferred/conflict/needs_human counts, two push payload records, one accepted row after conflict filtering, retry-to-human gate, and pull response decode; supporting evidence only, no real backend or signed runtime |
...
| desktop unsigned release app+DMG build | exit `0` in unsandboxed macOS environment; unsigned release `.app` and `desktop/target/release/bundle/dmg/安心智能助手_1.0.0_aarch64.dmg` were built; earlier sandbox failure is attributed to blocked `hdiutil` / DiskManagement access, not product packaging logic | `docs/release/evidence/artifacts/desktop-release-unsigned-local-build-20260507.json` |
