...
| Mobile remote-control safety gate | code-level complete for persisted pairing requests, desktop confirmation, DB-backed route-token issuance, command queue, cancellation, redacted audit events, and fail-closed local/top-secret/missing-token/high-risk states; still no real desktop host pickup/status callback/device evidence | `backend/src/services/remote_control_service.py`, `backend/src/api/routes/sync.py`, `backend/src/models/sync.py`, `backend/alembic/versions/042_add_remote_control_queue.py`, `backend/tests/test_remote_control_fail_closed.py`, `mobile/src/app/desktop-control.tsx` |
