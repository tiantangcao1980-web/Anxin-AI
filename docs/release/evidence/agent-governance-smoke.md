...
| Route-token broker | code-level complete for hash-only lease storage, scope checks, expiration, consumer mismatch, revocation next-call failure, and audit events | `backend/tests/test_agent_governance_service.py`, `backend/tests/test_mcp_route_governance.py`, `backend/tests/test_cli_route.py`, `backend/tests/test_llm_route_governance.py`, `backend/tests/test_crawler_compliance.py` |
...
| LLM/browser/desktop-control route-token integration | code-level partial for REST/SSE/WebSocket Chat Agent LLM runtime and browser/crawler fetch: Agent sync/stream model calls now fail closed in staging/production unless an org-scoped DB-backed route token with `llm:chat` scope is supplied, `/chat/route-token` can issue a user-bound token, WebSocket auth/header/message payloads pass route credentials into Agent/Coordinator calls, and `crawler_service.fetch()` requires `browser:fetch` before robots/http access. Route revocation denies the next Agent LLM/crawler call and writes governance audit events. RAG direct LLM calls, full browser automation beyond the crawler fetch path, desktop-control high-risk execution, real connector rehearsal, and cross-process runtime evidence remain pending. | `backend/src/services/llm_route_governance.py`, `backend/src/agents/base.py`, `backend/src/api/routes/chat.py`, `backend/tests/test_llm_route_governance.py`, `backend/src/services/crawler_service.py`, `backend/src/core/config.py`, `backend/tests/test_crawler_compliance.py`, `scripts/commercial-readiness-gate.sh` |
