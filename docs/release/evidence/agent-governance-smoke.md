...
| Route-token broker | code-level complete for hash-only lease storage, scope checks, expiration, consumer mismatch, revocation next-call failure, and audit events | `backend/tests/test_agent_governance_service.py`, `backend/tests/test_mcp_route_governance.py`, `backend/tests/test_cli_route.py`, `backend/tests/test_crawler_compliance.py` |
...
| LLM/browser/desktop-control route-token integration | code-level partial for browser/crawler fetch: `crawler_service.fetch()` now fails closed in staging/production before robots or HTTP access unless an org-scoped DB-backed route token with `browser:fetch` scope is supplied; route revocation denies the next fetch and writes governance audit events. Full LLM runtime, browser automation beyond the crawler fetch path, desktop-control high-risk execution, real connector rehearsal, and cross-process runtime evidence remain pending. | `backend/src/services/crawler_service.py`, `backend/src/core/config.py`, `backend/tests/test_crawler_compliance.py`, `scripts/commercial-readiness-gate.sh` |
