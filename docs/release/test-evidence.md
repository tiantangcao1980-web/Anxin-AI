...
- 可信会话与 Skills 进化：本地 SkillEvolutionProposal/eval gate/审批/灰度/回滚测试已补，AgentApproval/AgentAuditEvent 模型和 AgentApprovalService 已补；下一步补长任务事件流、任务时间线、工具状态、artifact 编辑、暂停/恢复/接管、跨设备恢复、SkillGovernance 持久化/记忆治理和真实执行链路失权测试。
