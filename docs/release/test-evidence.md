...
2026-05-08 补充：REST/SSE/WebSocket Chat Agent LLM runtime 已在商业环境加入 DB-backed `llm:chat` route-token gate，`/chat/route-token` 可按当前用户 consumer 签发短期 token，WebSocket 可从 auth 首包、连接 header 或单条消息 payload 透传 route-token，Agent 同步/流式模型调用在真实 HTTP 前校验；`backend/tests/test_llm_route_governance.py` 当前 `8 passed`，覆盖缺 token、DB token 放行、route revoke、流式触网前拒绝、WebSocket route-token 透传和用户绑定签发。RAG direct LLM、完整 browser automation 和 desktop-control runtime 仍待闭环。

2026-05-08 补充：`crawler_service.fetch()` 已在商业环境对浏览器/爬虫 fetch 加入 DB-backed `browser:fetch` route-token gate，授权发生在 robots/http 网络访问前；`backend/tests/test_crawler_compliance.py` 当前 `7 passed`，覆盖缺 token fail-closed、DB route-token 放行、route revoke 后下一次触网前拒绝。该证据只关闭 browser/crawler fetch 入口，不替代 RAG direct LLM、完整 browser automation、desktop-control 和真实 connector runtime 证据。
