...
- 这仍不是远控完成态：当前只证明后端 host lifecycle API、桌面 IPC host 客户端和审计边界，不证明常驻后台自动轮询、高风险真实桌面执行、跨设备连续会话、真实移动设备或 signed desktop runtime。
