...
# frontend build, frontend workstation settings Playwright e2e 9 passed / 1 skipped, frontend agent approval workspace Playwright e2e 2 passed / 2 skipped, desktop cargo check, desktop cargo test 21 passed, desktop installed-profile SQLCipher/keyring smoke, backend mypy zero baseline now 0/0, agent governance policy tests 4 passed, capability route token tests 4 passed, agent governance model tests 8 passed, agent governance service tests 6 passed, agent approval service tests 5 passed, agent approval API tests 4 passed, MCP route governance tests 5 passed, CLI route governance tests 10 passed, approval authorization guard tests 7 passed, skill evolution gate tests 9 passed, RAG full50 runner tests 8 passed,
...
# mobile-device-smoke 8 files / 27 tests + mobile/mini tsc + mobile result-surface/lawyer-conversion guards + Expo config/SDK guard + mini privacy/navigation boundary + weapp build + WeChat DevTools CLI + refresh-auth/mobile+mini privacy/fake-fallback/design-token guards.
...
- AgentApproval API 已补正式后端契约：`POST /agent-approvals`、列表/详情、`/pending/count`、`/{id}/audit-events`、`/{id}/audit-export`、approve/reject/revoke 和 validate 覆盖未登录拒绝、组织/本人 scope、普通员工自批拒绝、管理员批准/撤销、执行前放行/拒绝、审批审计时间线、审批审计 JSON 导出和 payload 脱敏；该切片已加入 `scripts/commercial-readiness-gate.sh --with-local-tests`。
...
- `bash scripts/mobile-device-smoke.sh --out docs/release/evidence/artifacts/mobile-mini-code-smoke-20260508.json --manual-template-out docs/release/evidence/artifacts/mobile-device-manual-template-20260508.json` -> mobile Vitest `8 files / 27 tests passed`; mobile tsc `0`; mobile result surface guard `0`; mobile lawyer conversion guard `0`; Expo config/SDK guard `0`; Expo doctor `17/17`; mobile npm audit `critical=0/high=0/total=0`; mini-program tsc `0`; mini-program privacy boundary guard `0`; mini-program navigation boundary guard `0`; mini-program `build:weapp` `0`; WeChat DevTools CLI project smoke `0`; refresh auth guard `0`; mobile/mini privacy network guard `0`; fake fallback guard `0`; mini-program design token guard `0`; code-level JSON artifact and manual device evidence template written
...
- 可信会话与 Skills 进化：本地 SkillEvolutionProposal/eval gate/审批/灰度/回滚测试已补，AgentApproval/AgentAuditEvent 模型、AgentApprovalService、`/agent-approvals` API 和最小前端审批工作台已补；下一步补长任务事件流、任务时间线、工具状态、artifact 编辑、暂停/恢复/接管、跨设备恢复、SkillGovernance 持久化/记忆治理和真实执行链路失权测试。
