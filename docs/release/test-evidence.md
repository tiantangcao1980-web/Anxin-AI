...
- 仍不能把桌面主工作站称为完成态：缺 approved MCP connector 演练、移动远控设备配对、signed packaged runtime UI/出站复验证据。
...
- 这只关闭最小桌面配置写入面；真实 approved MCP connector 演练、signed packaged runtime 和跨设备真机证据仍保持阻断。
...
- 这关闭的是桌面主工作站“环境 profile”本地 CRUD；真实 LLM/MCP/Skills connector 密钥联调、真实 approved connector 演练、signed runtime 和跨设备真机证据仍保持阻断。
...
- 这关闭的是 LLM provider/endpoint/model/key CRUD 的组织隔离和遮罩边界；真实 provider 密钥联调、route-token runtime 证据、完整前端配置体验和 approved connector 演练仍保持阻断。
