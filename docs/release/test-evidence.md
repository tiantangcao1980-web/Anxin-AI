...
- `mobile/app/desktop-control.tsx`、`mobile/src/services/api.ts` 和 `mobile/src/features/desktop-control/model.ts` 新增移动端 safe-probe 入队面：只有 confirmed pairing 同时返回 `desktop_device_id` 与 `pairing_id` 时才启用；点击后先申请短期 route token，再只下发 `desktop.status_probe` / `l2` / 120 秒过期命令；local 模式仍在 fetch 前 fail-closed；UI 显示“入队等待桌面 host 回传”，不声明已执行。
