...
- 仍不能把桌面主工作站称为完成态：缺 LLM/MCP/Skills connector 凭据 CRUD、approved MCP connector 演练、移动远控设备配对、signed packaged runtime UI/出站复验证据。
...
- 全设备智能助手：桌面主工作站最小入口、TopSecret-safe action model、浏览器级 Settings 工作站路由/移动宽度证据、本地 LLM/模型数量/离线队列/知识库/MCP 只读探针、secret-free 环境配置档 CRUD、移动远控 DB-backed 控制面、后端 host lifecycle API、桌面 IPC host 客户端、bounded safe-probe poll 和移动端状态入口已补；下一步补完整 LLM/MCP/Skills connector 凭据 CRUD、真正常驻后台 daemon 与高风险真实执行器、真实移动远控跨设备联调、approved connector 演练和 signed runtime 出站 fail-closed 测试。
