...
2026-05-08 本轮移动远控控制面切片：

```bash
cd backend && ./.venv/bin/pytest -q tests/test_remote_control_fail_closed.py
# 8 passed

cd backend && ./.venv/bin/ruff check src/api/routes/sync.py src/models/sync.py src/services/remote_control_service.py tests/test_remote_control_fail_closed.py
# All checks passed
```

覆盖增量：

- `backend/src/models/sync.py`、`backend/alembic/versions/042_add_remote_control_queue.py` 和 `backend/src/services/remote_control_service.py` 新增 DB-backed 配对、命令队列和审计事件控制面。
...
- 这仍不是远控完成态：当前只证明控制面、队列和审计边界，不证明桌面 host 已拉取/执行命令，也不证明状态回传、跨设备连续会话、真实移动设备或 signed desktop runtime。
...
- 全设备智能助手：桌面主工作站最小入口、TopSecret-safe action model、浏览器级 Settings 工作站路由/移动宽度证据、本地 LLM/模型数量/离线队列/知识库/MCP 只读探针、移动远控 DB-backed 控制面和移动端状态入口已补；下一步补完整配置 CRUD、桌面 host 拉取/执行/状态回传、真实移动远控跨设备联调、任意 LLM/Skills/MCP 配置、approved connector 演练和 signed runtime 出站 fail-closed 测试。
