...
- 可信会话与 Skills 进化：本地 SkillEvolutionProposal/eval gate/审批/灰度/回滚测试已补，AgentApproval/AgentAuditEvent 模型、AgentApprovalService、`/agent-approvals` API 和最小前端审批工作台已补，workspace-control 在真实运行时未接入前已 fail-closed；下一步补长任务事件流、任务时间线、工具状态、artifact 编辑、真实暂停/恢复/接管/终止执行效果、跨设备恢复、SkillGovernance 持久化/记忆治理和真实执行链路失权测试。
