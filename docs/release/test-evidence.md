...
- 仍不能把绝密模式称为全链路完成：WebView 内大量业务 API 仍需统一入口治理、CSP/tauri-plugin-http 策略和 packaged runtime 出站拦截证据。
