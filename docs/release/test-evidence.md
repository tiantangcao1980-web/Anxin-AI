...
- 仍不能把桌面主工作站称为完成态：缺 Skills connector 前端凭据体验、approved MCP connector 演练、移动远控设备配对、signed packaged runtime UI/出站复验证据。
...
- 这只关闭最小桌面配置写入面；Skills connector 前端凭据体验、真实 approved MCP connector 演练、signed packaged runtime 和跨设备真机证据仍保持阻断。
...
- 这只关闭桌面工作站模式/后端环境的跨重启持久化；第三方 connector 前端凭据体验、真实密钥联调、signed packaged runtime 和跨设备证据仍保持阻断。
...
- 这关闭的是桌面主工作站“环境 profile”本地 CRUD；完整 LLM/MCP/Skills connector 凭据管理、真实 approved connector 演练、signed runtime 和跨设备真机证据仍保持阻断。
...
- 全设备智能助手：桌面主工作站最小入口、TopSecret-safe action model、浏览器级 Settings 工作站路由/移动宽度证据、本地 LLM/模型数量/离线队列/知识库/MCP 只读探针、secret-free 环境配置档 CRUD、LLM 配置组织级 CRUD/API key 遮罩边界、前端 LLM 凭据遮罩/保留/新增体验、后台模型配置组织级入口、Skills connector 后端组织级凭据 CRUD/credential_keys 遮罩/审计脱敏、移动远控 DB-backed 控制面、后端 host lifecycle API、桌面 IPC host 客户端、bounded safe-probe poll 和移动端状态入口已补；下一步补 Skills connector 前端凭据体验、真实 provider 密钥联调、真正常驻后台 daemon 与高风险真实执行器、真实移动远控跨设备联调、approved connector 演练和 signed runtime 出站 fail-closed 测试。
