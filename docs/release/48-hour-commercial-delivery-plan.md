...
> 日期：2026-05-08
> 基线：当前仓库的代码级门禁大多已过，RAG 内建 full50 已闭合，前端和移动 production dependency audit 已清零；真正阻断点集中在支付/电签真实资源、桌面 signed/notarized package 与 packaged runtime、移动真机/交互式验证，以及当前工作树整理。
...
| 维度 | 当前事实 | 48 小时内处理口径 |
|---|---|---|
| GitNexus | commit-scoped index 为 `1181 files / 30003 nodes / 54587 edges / 300 flows / 28219 embeddings`，vector gate 已过；当前 dirty worktree 仍为 critical 影响面 | 本轮开发继续用 GitNexus direct CLI、`rg` 和测试互证；提交前不把索引当作完整覆盖；提交后必须重跑 GitNexus |
| release artifacts | `python3 scripts/validate-release-artifacts.py` -> `PASS (24 JSON artifacts)`；`bash scripts/release-evidence-secret-scan.sh` -> `PASS`；`git diff --check` -> `PASS` | 新增真实证据后立即复跑 artifact validator 和 secret scan |
| 工作树 | `release-worktree-inventory` 当前 `tracked_changes=380`、`untracked_files=307`、`release_delivery=686`、`generated_or_runtime=1`、`local_secret=0`、`unknown=0` | 42-48h 必须整理并提交可追踪交付文件；`frontend/dist/index.html` 这个 generated/runtime 路径需单独确认 |
...
| 移动/小程序 | 2026-05-08 复跑 `scripts/mobile-device-smoke.sh` 通过：mobile `7 files / 17 tests`、tsc、Expo doctor `17/17`、mobile npm audit `0`、mini tsc/build、WeChat DevTools CLI project smoke、refresh/fallback/design-token guards，并写出 `mobile-mini-code-smoke-20260508.json` / `mobile-device-manual-template-20260508.json` | 代码级已稳；商业证据还缺 iOS/Android/交互式 WeChat DevTools 或真机、跨设备连续会话 |
| 商业门禁 | `commercial-readiness-gate.sh --quick` 仍 FAIL：dirty worktree、Not ready 声明、payment/e-sign/desktop/mobile evidence pending | 失败是正确防线；不能改状态绕过，必须补真实证据或保留 blocker |
...
| 本地可完成 | 跑 `release-worktree-inventory`、secret scan、static quality baseline，确认当前只剩已知阻断；把支付、电签、桌面、移动的证据文件状态逐项复核；RAG 内建 full50 证据保持 complete |
...
当前执行快照：`release-worktree-inventory` 已确认 `unknown=0`、`local_secret=0`；前端和移动 production dependency audit 均已清零；支付/电签 runner 已补 live honest-gating，退款 `pending`、关单/取消 `False`、无 signer URL、空签署文件下载不再被误报为完成；artifact validator、release evidence secret scan 和 `git diff --check` 当前通过；`commercial-readiness-gate.sh --quick` 仍正确失败于 dirty worktree、Not ready 声明和 payment/e-sign/desktop/mobile evidence pending。
...
| 退出标准 | iOS / Android / WeChat / 桌面 的真实证据已分别落盘；所有 evidence 文件只保留真实 artifact reference；任何仍缺的 lane 必须明确写成 blocker |
