...
| 维度 | 当前事实 | 48 小时内处理口径 |
|---|---|---|
| GitNexus | commit-scoped index 已在本轮本地交付提交后重建；精确统计以最新 `.gitnexus/meta.json` 与 `gitnexus cypher` 计数为准，vector gate 要求 meta/Cypher embeddings 非零且一致；历史 dirty snapshot 曾为 `380 files / 6037 symbols / critical`，当前不能把该历史影响面当作最新工作树结论 | 本轮开发继续用 GitNexus direct CLI、`rg` 和测试互证；后续新增提交后必须重跑 GitNexus |
...
| 移动/小程序 | 2026-05-08 复跑 `scripts/mobile-device-smoke.sh` 通过：mobile `7 files / 17 tests`、tsc、Expo doctor `17/17`、mobile npm audit `0`、mini tsc/build、WeChat DevTools CLI project smoke、refresh/fallback/design-token guards，并写出 `mobile-mini-code-smoke-20260508.json` / `mobile-device-manual-template-20260508.json`；host probe 更新为 `mobile-device-host-probe-20260508.json`，确认 iOS Simulator 可用、Android device/emulator 仍缺 | 代码级已稳；商业证据还缺 iOS app-run、Android、交互式 WeChat DevTools 或真机、跨设备连续会话、移动远控桌面 |
