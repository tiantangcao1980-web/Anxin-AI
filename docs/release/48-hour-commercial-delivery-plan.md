...
| 移动/小程序 | 2026-05-08 复跑 `scripts/mobile-device-smoke.sh` 通过：mobile `7 files / 19 tests`、tsc、Expo doctor `17/17`、mobile npm audit `0`、mini tsc/build、mini privacy boundary guard、WeChat DevTools CLI project smoke、refresh/mobile+mini privacy/fallback/design-token guards，并写出 `mobile-mini-code-smoke-20260508.json` / `mobile-device-manual-template-20260508.json`；移动请求层已透传 `X-Privacy-Mode` 且 local 模式 fetch 前 fail-closed；小程序请求层已透传 `X-Privacy-Mode`，local/top-secret 在数据请求和登录前 fail-closed；host probe 更新为 `mobile-device-host-probe-20260508.json`，确认 iOS Simulator 可用、Android device/emulator 仍缺 | 代码级已稳；商业证据还缺 iOS app-run、Android、交互式 WeChat DevTools 或真机、跨设备连续会话、移动远控桌面 |
