...
| release artifacts | `python3 scripts/validate-release-artifacts.py` -> `PASS (29 JSON artifacts)`；`bash scripts/release-evidence-secret-scan.sh` -> `PASS`；`git diff --check` -> `PASS` | 新增真实证据后立即复跑 artifact validator 和 secret scan |
