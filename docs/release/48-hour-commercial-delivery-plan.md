...
| 全设备智能助手 | LLM/MCP/Skills/knowledge/local LLM 底座已在后端、桌面和移动隐私上下文中存在；桌面工作站已补本地 LLM/模型数量/离线队列、知识库数量/文档数、MCP 服务/启用/工具缓存只读探针，并把移动远控保持为安全闸/待验收；2026-05-08 OpenSpec 已把桌面主工作站、移动远控、任意模型/Skills/MCP、独立知识库和本地模型安全列为验收 | 48 小时内先冻结规范和证据模板；真实交付需补配置 UI、组织权限回归、approved MCP connector 演练、远控协议和 signed 绝密模式出站拦截证据 |
