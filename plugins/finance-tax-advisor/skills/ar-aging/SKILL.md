---
name: ar-aging
description: 应收账款账龄分析 + 催收优先级 + 法律风险账款标记。
access-level: practice
data-classification: L4
jurisdiction: CN
required-scopes:
  - skill.finance-tax-advisor.ar_aging
  - data.finance.read
tool-allowlist:
  - read_only
  - read_write_local
...
# /finance-tax-advisor:ar-aging

应收账款账龄分析 + 催收优先级 + 法律风险账款标记。
...
4. **草稿产出** — 渲染成 `xlsx` 草稿，写入 `.claude/drafts/finance-tax-advisor/ar-aging-<ts>.xlsx`。
...
- Skill registry: `backend/src/skills/registry.py` (entry `ar-aging`)
- 单元测试: `backend/tests/test_personas_finance_tax_advisor_ar_aging.py`
