---
name: review
description: 智能合同审查：识别风险条款、关键商业条款，生成专业修改建议。覆盖 10+ 合同类型。
access-level: practice
data-classification: L3
jurisdiction: CN
required-scopes:
  - skill.contract-steward.review
  - data.legal.read
tool-allowlist:
  - read_only
  - llm_call
...
智能合同审查：识别风险条款、关键商业条款，生成专业修改建议。覆盖 10+ 合同类型。
...
2. **调用底层能力**
     - 复用 [`skills/legal/contract-review/SKILL.md`](../../../../skills/legal/contract-review/SKILL.md) 中定义的底层算法 / 规则集 / Prompt 模板。
3. **应用执业画像** — 套用 `CLAUDE.md` 的术语、文风、升级阈值。
4. **草稿产出** — 渲染成 `docx` 草稿，写入 `.claude/drafts/contract-steward/review-<ts>.docx`。
...
- Skill registry: `backend/src/skills/registry.py` (entry `review`)
- 单元测试: `backend/tests/test_personas_contract_steward_review.py`
