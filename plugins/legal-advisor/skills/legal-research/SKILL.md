---
name: legal-research
description: 中国法律法规检索 + 案例摘要 + 风险评级。结合人大法规库、裁判文书、信用中国。
access-level: practice
data-classification: L3
jurisdiction: CN
required-scopes:
  - skill.legal-advisor.legal_research
  - data.legal.read
tool-allowlist:
  - read_only
  - llm_call
...
中国法律法规检索 + 案例摘要 + 风险评级。结合人大法规库、裁判文书、信用中国。
...
2. **调用底层能力**
     - 复用 [`skills/legal/legal-research/SKILL.md`](../../../../skills/legal/legal-research/SKILL.md) 中定义的底层算法 / 规则集 / Prompt 模板。
3. **应用执业画像** — 套用 `CLAUDE.md` 的术语、文风、升级阈值。
4. **草稿产出** — 渲染成 `md` 草稿，写入 `.claude/drafts/legal-advisor/legal-research-<ts>.md`。
...
- Skill registry: `backend/src/skills/registry.py` (entry `legal-research`)
- 单元测试: `backend/tests/test_personas_legal_advisor_legal_research.py`
