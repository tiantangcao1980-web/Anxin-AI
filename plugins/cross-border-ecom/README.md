...
> **选品 / 独立站 / Amazon / VAT** · 业务域：出海跨境

## 这是什么

`cross-border-ecom` 是 [安心智能助手](../../README.md) 的 Persona 插件之一，提供 *选品 / 独立站 / Amazon / VAT* 能力。本插件已在 V3 主干注册到 `backend/src/agents/personas/registry.py`，可在桌面端 / Web / 飞书 / 钉钉同步调用。
...
# 2. 安装本插件
/plugin install cross-border-ecom@anxin-ai

# 3. 第一次使用前，跑冷启动访谈（生成 CLAUDE.md 执业画像）
/cross-border-ecom:cold-start-interview
```
...
| 命令 | 说明 |
|---|---|
| `/cross-border-ecom:product-source` | product-source skill |
| `/cross-border-ecom:factory-audit` | factory-audit skill |
| `/cross-border-ecom:amazon-listing` | amazon-listing skill |
| `/cross-border-ecom:vat-filing` | vat-filing skill |
| `/cross-border-ecom:cold-start-interview` | 首次冷启动访谈，生成执业画像 |
