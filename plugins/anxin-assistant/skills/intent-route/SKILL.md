---
name: intent-route
description: 自然语言意图识别 + 路由到对应 persona；支持多 persona 协作 DAG。
access-level: practice
data-classification: L2
jurisdiction: CN
required-scopes:
  - skill.anxin-assistant.intent_route
tool-allowlist:
  - read_only
  - llm_call
...
# /anxin-assistant:intent-route

自然语言意图识别 + 路由到对应 persona；支持多 persona 协作 DAG。
...
4. **草稿产出** — 渲染成 `md` 草稿，写入 `.claude/drafts/anxin-assistant/intent-route-<ts>.md`。
...
- Skill registry: `backend/src/skills/registry.py` (entry `intent-route`)
- 单元测试: `backend/tests/test_personas_anxin_assistant_intent_route.py`
