...
`anxin-assistant` 是 [安心智能助手](../../README.md) 的 Persona 插件之一，提供 *通用入口与任务编排* 能力。本插件已在 V3 主干注册到 `backend/src/agents/personas/registry.py`，可在桌面端 / Web / 飞书 / 钉钉同步调用。
...
# 2. 安装本插件
/plugin install anxin-assistant@anxin-ai

# 3. 第一次使用前，跑冷启动访谈（生成 CLAUDE.md 执业画像）
/anxin-assistant:cold-start-interview
```
...
| 命令 | 说明 |
|---|---|
| `/anxin-assistant:intent-route` | intent-route skill |
| `/anxin-assistant:task-decompose` | task-decompose skill |
| `/anxin-assistant:consensus-vote` | consensus-vote skill |
| `/anxin-assistant:cold-start-interview` | 首次冷启动访谈，生成执业画像 |
