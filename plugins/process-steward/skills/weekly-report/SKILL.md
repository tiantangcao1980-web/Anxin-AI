---
name: weekly-report
description: 周报自动生成：聚合飞书 / 钉钉 / GitHub / Linear 多源动态，按 OKR 维度结构化输出。
access-level: practice
data-classification: L2
jurisdiction: CN
required-scopes:
  - skill.process-steward.weekly_report
tool-allowlist:
  - read_only
  - llm_call
...
# /process-steward:weekly-report

周报自动生成：聚合飞书 / 钉钉 / GitHub / Linear 多源动态，按 OKR 维度结构化输出。
...
4. **草稿产出** — 渲染成 `md` 草稿，写入 `.claude/drafts/process-steward/weekly-report-<ts>.md`。
...
- Skill registry: `backend/src/skills/registry.py` (entry `weekly-report`)
- 单元测试: `backend/tests/test_personas_process_steward_weekly_report.py`
