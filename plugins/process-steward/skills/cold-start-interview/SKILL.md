...
description: 首次使用 流程管家 前必跑的执业画像访谈 — 学习你的团队规模 / 行业 / SOP / 风险偏好，并写入 plugins/process-steward/CLAUDE.md。
access-level: practice
data-classification: L2
jurisdiction: CN
required-scopes:
  - skill.process-steward.cold_start_interview
tool-allowlist:
  - read_only
  - read_write_local
  - llm_call
...
为新接入的团队建立 **流程管家** 专属执业画像（`plugins/process-steward/CLAUDE.md`）。此后所有 skill 都会从该文件读取你的行业、口径、红线、术语，避免输出像通用 ChatGPT 一样空泛。

> **重要**：在没有跑过本访谈前，所有 `/process-steward:*` 命令都会输出风险提示并尽量保守。这与 Anthropic 在 *Claude for Legal* 与 *Claude for Financial Services* 中的 cold-start 设计同源。
...
写入 `plugins/process-steward/CLAUDE.md`，并在 `MEMORY.md` 增加一条记忆指针 `- [process-steward practice profile](plugins/process-steward/CLAUDE.md) — Cold-start 完成于 {date}`。
