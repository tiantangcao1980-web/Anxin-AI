...
> **OKR / 周报 / 会议 / 审批** · 业务域：综合协调

## 这是什么

`process-steward` 是 [安心智能助手](../../README.md) 的 Persona 插件之一，提供 *OKR / 周报 / 会议 / 审批* 能力。本插件已在 V3 主干注册到 `backend/src/agents/personas/registry.py`，可在桌面端 / Web / 飞书 / 钉钉同步调用。
...
# 2. 安装本插件
/plugin install process-steward@anxin-ai

# 3. 第一次使用前，跑冷启动访谈（生成 CLAUDE.md 执业画像）
/process-steward:cold-start-interview
```
...
| 命令 | 说明 |
|---|---|
| `/process-steward:okr-track` | okr-track skill |
| `/process-steward:meeting-minutes` | meeting-minutes skill |
| `/process-steward:weekly-report` | weekly-report skill |
| `/process-steward:approval-route` | approval-route skill |
| `/process-steward:cold-start-interview` | 首次冷启动访谈，生成执业画像 |
