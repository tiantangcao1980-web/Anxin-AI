---
name: asset-trace
description: 资产追溯：识别股权 / 不动产 / 银行 / 关联交易资产线索，输出可执行追索路径。
access-level: practice
data-classification: L4
jurisdiction: CN
required-scopes:
  - skill.dd-expert.asset_trace
  - data.legal.read
tool-allowlist:
  - read_only
  - llm_call
...
# /dd-expert:asset-trace

资产追溯：识别股权 / 不动产 / 银行 / 关联交易资产线索，输出可执行追索路径。
...
2. **调用底层能力**
     - 复用 [`skills/legal/asset-tracing/SKILL.md`](../../../../skills/legal/asset-tracing/SKILL.md) 中定义的底层算法 / 规则集 / Prompt 模板。
3. **应用执业画像** — 套用 `CLAUDE.md` 的术语、文风、升级阈值。
4. **草稿产出** — 渲染成 `md` 草稿，写入 `.claude/drafts/dd-expert/asset-trace-<ts>.md`。
...
- Skill registry: `backend/src/skills/registry.py` (entry `asset-trace`)
- 单元测试: `backend/tests/test_personas_dd_expert_asset_trace.py`
