...
> **公司/项目/客户 360° 尽调** · 业务域：合规经营

## 这是什么

`dd-expert` 是 [安心智能助手](../../README.md) 的 Persona 插件之一，提供 *公司/项目/客户 360° 尽调* 能力。本插件已在 V3 主干注册到 `backend/src/agents/personas/registry.py`，可在桌面端 / Web / 飞书 / 钉钉同步调用。
...
# 2. 安装本插件
/plugin install dd-expert@anxin-ai

# 3. 第一次使用前，跑冷启动访谈（生成 CLAUDE.md 执业画像）
/dd-expert:cold-start-interview
```
...
| 命令 | 说明 |
|---|---|
| `/dd-expert:company-dd` | company-dd skill |
| `/dd-expert:judicial-search` | judicial-search skill |
| `/dd-expert:ip-portfolio-check` | ip-portfolio-check skill |
| `/dd-expert:credit-trace` | credit-trace skill |
| `/dd-expert:cold-start-interview` | 首次冷启动访谈，生成执业画像 |
