---
name: lead-score
description: 线索评分：BANT + 自定义权重 + 历史成单数据训练，输出 0-100 评分 + 推荐动作。
access-level: practice
data-classification: L2
jurisdiction: CN
required-scopes:
  - skill.lead-hunter.lead_score
tool-allowlist:
  - read_only
  - network_egress
  - llm_call
...
# /lead-hunter:lead-score

线索评分：BANT + 自定义权重 + 历史成单数据训练，输出 0-100 评分 + 推荐动作。
...
4. **草稿产出** — 渲染成 `xlsx` 草稿，写入 `.claude/drafts/lead-hunter/lead-score-<ts>.xlsx`。
...
- Skill registry: `backend/src/skills/registry.py` (entry `lead-score`)
- 单元测试: `backend/tests/test_personas_lead_hunter_lead_score.py`
