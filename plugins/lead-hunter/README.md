...
> **线索 + 评分 + 触达 + Vibe Selling** · 业务域：增长获客

## 这是什么

`lead-hunter` 是 [安心智能助手](../../README.md) 的 Persona 插件之一，提供 *线索 + 评分 + 触达 + Vibe Selling* 能力。本插件已在 V3 主干注册到 `backend/src/agents/personas/registry.py`，可在桌面端 / Web / 飞书 / 钉钉同步调用。
...
# 2. 安装本插件
/plugin install lead-hunter@anxin-ai

# 3. 第一次使用前，跑冷启动访谈（生成 CLAUDE.md 执业画像）
/lead-hunter:cold-start-interview
```
...
| 命令 | 说明 |
|---|---|
| `/lead-hunter:lead-source` | lead-source skill |
| `/lead-hunter:lead-score` | lead-score skill |
| `/lead-hunter:email-outreach` | email-outreach skill |
| `/lead-hunter:quote-generate` | quote-generate skill |
| `/lead-hunter:cold-start-interview` | 首次冷启动访谈，生成执业画像 |
