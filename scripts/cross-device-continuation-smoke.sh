...
# DevTools interaction, or a shared staging account session.
#
# 2026-05: 删除 uni-mobile 客户端契约步骤 — UniApp 路线已终止，
# 移动端契约统一由 mobile/ (Expo RN) + mini-program/ (Taro) 承担。
...
report = {
    ...
    "completion_note": (
        ...
        "device runs, interactive WeChat DevTools evidence, and a shared staging "
        "account cross-device continuation transcript are attached."
    ),
}
