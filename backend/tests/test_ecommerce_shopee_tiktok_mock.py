...
async def test_shopee_country_filter_narrows_results():
    src = ShopeeEcommerceSource()
    vn_results = await src.search_products(ProductSearchQuery(keyword="x", country="VN"))
...
async def test_shopee_health_check_requires_partner_credentials():
    ...
    assert await ShopeeEcommerceSource(partner_id="123", partner_key="key").health_check() is True
...
async def test_tiktok_sort_newest_orders_by_viral_views_desc():
    ...
    src = TikTokShopEcommerceSource()
    results = await src.search_products(ProductSearchQuery(keyword="x", sort_by="newest"))
...
async def test_tiktok_sort_price_asc():
    src = TikTokShopEcommerceSource()
    results = await src.search_products(ProductSearchQuery(keyword="x", sort_by="price_asc"))
...
async def test_tiktok_country_filter():
    src = TikTokShopEcommerceSource()
    uk_results = await src.search_products(ProductSearchQuery(keyword="x", country="UK"))
...
async def test_tiktok_health_check_requires_app_credentials():
    ...
    assert (
        await TikTokShopEcommerceSource(app_key="key", app_secret="secret").health_check() is True
    )
