...
def test_mcp_allowlist_settings_accept_json_env(monkeypatch):
    ...
    monkeypatch.setenv("LLM_ROUTE_TOKEN_REQUIRED", "true")
    monkeypatch.setenv("BROWSER_FETCH_ROUTE_TOKEN_REQUIRED", "true")
    ...
    assert settings.LLM_ROUTE_TOKEN_REQUIRED is True
    assert settings.BROWSER_FETCH_ROUTE_TOKEN_REQUIRED is True
