...
class TestCaseServiceEvents:
    ...
    async def test_get_timeline_uses_explicit_event_at_order(
        ...
        await service.add_event(test_case.id, "late", "较晚事件", event_at=late)
        await service.add_event(test_case.id, "early", "较早事件", event_at=early)

        timeline = await service.get_timeline(test_case.id)
        titles = [event.title for event in timeline[:2]]

        assert titles == ["较晚事件", "较早事件"]
...
class TestCaseServiceEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_update_case_rejects_invalid_status_transition(
        self,
        ...
        with pytest.raises(ValueError, match="无效案件状态转换"):
            await service.update_case(case_id=test_case.id, status="completed")

        await db_session.refresh(test_case)
        assert test_case.status == CaseStatus.PENDING

    @pytest.mark.asyncio
    async def test_update_case_terminal_status_is_read_only(
        self,
        db_session: AsyncSession,
        test_case: Case,
    ):
        ...
        with pytest.raises(ValueError, match="不允许修改"):
            await service.update_case(case_id=test_case.id, title="不应成功")

        await db_session.refresh(test_case)
        assert test_case.title != "不应成功"
