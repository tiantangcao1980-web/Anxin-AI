...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('org_id', sa.CHAR(36), sa.ForeignKey('organizations.id', ondelete='CASCADE'), nullable=False),
        sa.Column('name', sa.String(100), nullable=False, comment='团队名称'),
        sa.Column('description', sa.Text, nullable=True, comment='团队描述'),
        sa.Column('leader_id', sa.CHAR(36), sa.ForeignKey('users.id', ondelete='SET NULL'), nullable=True, comment='团队负责人'),
    ...
    op.create_table(
        'team_members',
        sa.Column('id', sa.CHAR(36), primary_key=True),
        sa.Column('team_id', sa.CHAR(36), sa.ForeignKey('teams.id', ondelete='CASCADE'), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('assignee_id', sa.CHAR(36), sa.ForeignKey('users.id', ondelete='CASCADE'), nullable=False),
        sa.Column('assigned_by', sa.CHAR(36), sa.ForeignKey('users.id', ondelete='SET NULL'), nullable=True, comment='分配人'),
    ...
    op.create_table(
        ...
        sa.Column('approved_by', sa.CHAR(36), sa.ForeignKey('users.id', ondelete='SET NULL'), nullable=True, comment='审批人'),
