...
class SentimentService:
    ...
    def __init__(self, db: AsyncSession) -> None:
        self.db = db
        self._agent: _SentimentAnalysisAgentProtocol | None = None

    @property
    def agent(self) -> _SentimentAnalysisAgentProtocol:
    ...
    async def get_monitor(self, monitor_id: str) -> SentimentMonitor | None:
        """获取监控配置详情"""
    ...
    async def toggle_monitor(self, monitor_id: str, is_active: bool) -> SentimentMonitor | None:
        """启用/禁用监控"""
    ...
    async def _update_monitor_stats(self, monitor_id: str, record: SentimentRecord) -> None:
        """更新监控统计信息"""
        result = await self.db.execute(
    ...
    async def get_record(self, record_id: str) -> SentimentRecord | None:
        """获取舆情记录详情"""
        result = await self.db.execute(
    ...
    async def mark_alert_read(self, alert_id: str) -> SentimentAlert | None:
        """标记预警已读"""
