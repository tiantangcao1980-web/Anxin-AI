...
class SearchCache(Base, TimestampMixin):
    ...
    )  # web_search / knowledge_base / business_registry / llm_analysis
...
class UserInvestigationPreference(Base, TimestampMixin):
    ...
    __table_args__ = (Index("ix_user_pref_user", "user_id", unique=True),)
    ...
    favorite_companies: Mapped[list[Any] | None] = mapped_column(
        JSON, nullable=True
    )  # [{name, count, last_investigated}]
    frequent_industries: Mapped[list[Any] | None] = mapped_column(
        JSON, nullable=True
    )  # [{industry, count}]
    frequent_regions: Mapped[list[Any] | None] = mapped_column(
        JSON, nullable=True
    )  # [{region, count}]
    ...
    # 用户行为画像
    avg_investigation_duration: Mapped[float | None] = mapped_column(
        Float, nullable=True
    )  # 平均每次调查耗时（秒）
    most_viewed_sections: Mapped[dict[str, Any] | None] = mapped_column(
        JSON, nullable=True
    )  # 最常查看的模块
    custom_dimensions: Mapped[list[Any] | None] = mapped_column(
        JSON, nullable=True
    )  # 用户自定义的调查维度
