...
class Subscription(Base, TimestampMixin):
    ...
    status: Mapped[str] = mapped_column(
        String(20), nullable=False, default="active", comment="订阅状态: pending/active/trial/past_due/cancelled/expired"
    )
...
class SubscriptionEvent(Base, TimestampMixin):
    ...
    from_status: Mapped[str | None] = mapped_column(
        String(20), nullable=True, comment="变更前状态"
    )
    to_status: Mapped[str] = mapped_column(
        String(20), nullable=False, comment="变更后状态"
    )
    event_type: Mapped[str] = mapped_column(
        String(50), nullable=False, comment="事件类型"
    )
    reason: Mapped[str | None] = mapped_column(
        String(256), nullable=True, comment="事件原因"
    )
