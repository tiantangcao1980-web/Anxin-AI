...
class ProfileRequest(BaseModel):
    ...
    law_firm: Optional[str] = Field(None, max_length=200, description="所在律所")
    ...
    hourly_rate_min: Optional[int] = Field(None, ge=0, description="最低时薪（元）")
    hourly_rate_max: Optional[int] = Field(None, ge=0, description="最高时薪（元）")


class CertificationRequest(BaseModel):
    license_image_url: str = Field(..., max_length=500, description="执业证照片URL")
    id_card_image_url: Optional[str] = Field(None, max_length=500, description="身份证照片URL")
    license_issue_date: date = Field(..., description="执业证签发日期")
    license_expiry_date: date = Field(..., description="执业证到期日期")
    bar_association: Optional[str] = Field(None, max_length=100, description="所属律师协会")
...
class ServiceConfigRequest(BaseModel):
    ...
    min_case_amount: Optional[float] = Field(None, ge=0, description="最低接案金额")


class VerifyCertificationRequest(BaseModel):
    approved: bool = Field(..., description="是否通过")
    rejection_reason: Optional[str] = Field(None, max_length=500, description="驳回原因")
