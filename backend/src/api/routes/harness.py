...
async def get_tool_stats(tool_name: Optional[str] = Query(None)):
    """获取工具调用统计"""
    from src.harness.tool_registry import tool_registry
...
async def get_policy_audit(limit: int = Query(50, le=200)):
    """获取权限审计日志"""
    from src.harness.policy_engine import policy_engine
...
async def get_user_tasks(user_id: str, limit: int = Query(20, le=100)):
    """获取用户任务列表"""
    from src.harness.task_engine import task_engine
