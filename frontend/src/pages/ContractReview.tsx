...
function getRiskLevelColor(level: string) {
 ...
 case'medium':
 return'bg-warning dark:bg-warning/30 text-warning border-yellow-200 dark:border-yellow-800';
 case'low':
...
function getRiskLevelIcon(level: string) {
 ...
 case'critical':
 case'high':
 return <icons.XCircle className={`${iconSize.md} text-destructive`} />;
 case'medium':
 return <icons.AlertTriangle className={`${iconSize.md} text-warning`} />;
 case'low':
 return <icons.CheckCircle className={`${iconSize.md} text-success`} />;
 default:
...
function renderContractText(
 ...
 <span
 key={`r${i}`}
 className="bg-success/10 text-success px-1 rounded cursor-pointer border-b-2 border-success/20 transition-colors hover:bg-success/20"
 ...
 <span
 key={`r${i}`}
 className="bg-destructive/10 text-destructive px-1 rounded cursor-pointer border-b-2 border-destructive/20 transition-colors hover:bg-destructive/20"
 onClick={() => onRiskClick(marker.riskIndex)}
 title="点击查看修改建议"
 >
 {marker.original}
...
export default function ContractReview({ embedded = false }: { embedded?: boolean }) {
 ...
 className={`h-full ${
 reviewResult.risk_score > 0.7 ?'bg-destructive' :
 reviewResult.risk_score > 0.5 ?'bg-orange-500' :
 reviewResult.risk_score > 0.3 ?'bg-warning' :'bg-success'
 }`}
 />
 </div>
 </div>
 ...
 <div className="flex items-center gap-2">
 <icons.AlertTriangle className={`${iconSize.sm} text-warning`} />
 ...
 {isAccepted && <icons.CheckCircle className={`${iconSize.md} text-success flex-shrink-0`} />}
 </div>
 ...
 <div className={`flex items-center gap-1.5 ${heading.micro} text-destructive font-medium mb-1`}>
 <icons.XCircle className={iconSize.sm} />
 原文
 </div>
 <p className="text-sm text-destructive line-through">{risk.original_text}</p>
 </div>

 {risk.suggested_text && (
 <div className="p-3 bg-success/10 border border-success/20 rounded-lg">
 <div className={`flex items-center gap-1.5 ${heading.micro} text-success font-medium mb-1`}>
 <icons.CheckCircle className={iconSize.sm} />
 建议修改
 </div>
 <p className="text-sm text-success">{risk.suggested_text}</p>
 </div>
 )}
 </div>
 )}
 ...
 <icons.CheckCircle className={`${iconSize['2xl']} mx-auto mb-3 text-success`} />
 <p>未检测到明显风险条款</p>
 </div>
 )}
 </div>
 ...
 <div className={`w-20 h-20 mx-auto mb-4 bg-success/10 ${radius.dialog} flex items-center justify-center`}>
 ...
 {risk.legal_basis && (
 <div className="flex items-start gap-2 p-2 mt-1.5 bg-info/10 rounded-lg border border-info/20">
 <icons.Scale className="w-4 h-4 text-info mt-0.5 flex-shrink-0" />
 <p className="text-xs text-info">{risk.legal_basis}</p>
 </div>
 )}
 {risk.suggestion && (
 ...
 <h4 className="font-medium text-sm text-warning mb-3 flex items-center gap-2">
