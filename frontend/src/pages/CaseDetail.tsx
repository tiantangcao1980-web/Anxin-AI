...
const PRIORITY_MAP: Record<string, { label: string; color: string }> = {
  low: { label: '低', color: 'bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400' },
  medium: { label: '中', color: 'bg-yellow-100 dark:bg-yellow-950/30 text-yellow-700 dark:text-yellow-400' },
  high: { label: '高', color: 'bg-orange-100 dark:bg-orange-950/30 text-orange-700 dark:text-orange-400' },
  urgent: { label: '紧急', color: 'bg-red-100 dark:bg-red-950/30 text-red-700 dark:text-red-400' },
}
            ...
            <div className="p-6 space-y-5">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <p className={`${heading.micro} font-medium uppercase tracking-wider`}>创建时间</p>
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <p className={`${heading.micro} font-medium uppercase tracking-wider`}>最后更新</p>
              ...
              {case_.risk_score !== null && case_.risk_score !== undefined && (
                <div className="pt-4 border-t">
                  <div className="flex justify-between items-end mb-2">
                    <p className={`${heading.micro} font-medium uppercase tracking-wider`}>AI 风险评分</p>
                    <p className={`text-lg font-bold ${case_.risk_score > 0.7 ? 'text-red-600' : case_.risk_score > 0.4 ? 'text-orange-600' : 'text-emerald-600'}`}>
                      {(case_.risk_score * 100).toFixed(0)}/100
                    </p>
                  </div>
                  <div className="h-2 w-full bg-muted rounded-full overflow-hidden">
                    <div 
                      className={`h-full transition-all duration-1000 ${case_.risk_score > 0.7 ? 'bg-red-500' : case_.risk_score > 0.4 ? 'bg-orange-500' : 'bg-emerald-500'}`}
