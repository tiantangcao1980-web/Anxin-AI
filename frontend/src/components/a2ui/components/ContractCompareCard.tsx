...
const changeTypeConfig = {
  added: { label: '新增', color: 'text-emerald-600 bg-emerald-50 border-emerald-200', icon: icons.CheckCircle },
  removed: { label: '删除', color: 'text-red-600 bg-red-50 border-red-200', icon: icons.XCircle },
  modified: { label: '修改', color: 'text-amber-600 bg-amber-50 border-amber-200', icon: icons.AlertTriangle },
            ...
            <div key={clause.id} className="group">
              ...
              >
                ...
                {clause.riskLevel && clause.riskLevel !== 'low' && (
                  <icons.AlertTriangle className={cn('w-3 h-3', clause.riskLevel === 'high' ? 'text-red-500' : 'text-amber-500')} />
                )}
                ...
                <div className="px-4 pb-3">
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className={cn(
                      ...
                      clause.changeType === 'removed' ? 'bg-red-50/50 text-red-800 line-through' : 'bg-muted text-foreground/80',
                    )}>
                    ...
                    <div className={cn(
                      'p-3 rounded-lg text-xs leading-relaxed',
                      clause.changeType === 'added' ? 'bg-emerald-50/50 text-emerald-800' :
                      clause.changeType === 'modified' ? 'bg-amber-50/50 text-amber-800' : 'bg-muted text-foreground/80',
                    )}>
