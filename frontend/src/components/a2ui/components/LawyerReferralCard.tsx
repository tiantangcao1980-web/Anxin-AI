...
export default function LawyerReferralCard({
 ...
 return (
 <Card className="border-warning/20 bg-gradient-to-br from-warning to-orange-50 dark:to-orange-900/20 mt-4">
 <CardHeader className="pb-2">
 <CardTitle className="text-base flex items-center gap-2">
 <icons.Briefcase className="h-5 w-5 text-warning" />
 专业律师协助
 </CardTitle>
 ...
 <p className="text-lg font-bold text-warning mb-2">{service.price}</p>
 <ul className="space-y-1">
 {service.features.map((f, i) => (
 <li key={i} className="text-xs text-muted-foreground flex items-center gap-1">
 <icons.CheckCircle className="h-3 w-3 text-success flex-shrink-0" />
 {f}
 </li>
 ))}
 </ul>
