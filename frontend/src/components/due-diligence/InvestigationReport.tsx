...
export function InvestigationReport({ companyName, investigationData, onBack }: InvestigationReportProps) {
  ...
  const statusIcon = (status?: string) => {
    switch (status) {
      case 'pass': return <icons.CheckCircle className="w-4 h-4 text-emerald-500" />
      case 'warning': return <icons.AlertCircle className="w-4 h-4 text-amber-500" />
      case 'fail': return <icons.XCircle className="w-4 h-4 text-red-500" />
      default: return null
    }
  }
