...
const SEVERITY_CONFIG = {
  low: { label: '低', color: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-50 dark:bg-emerald-950/30', border: 'border-emerald-200 dark:border-emerald-800' },
  medium: { label: '中', color: 'text-amber-600 dark:text-amber-400', bg: 'bg-amber-50 dark:bg-amber-950/30', border: 'border-amber-200 dark:border-amber-800' },
  high: { label: '高', color: 'text-red-600 dark:text-red-400', bg: 'bg-red-50 dark:bg-red-950/30', border: 'border-red-200 dark:border-red-800' },
  critical: { label: '极高', color: 'text-red-700 dark:text-red-300', bg: 'bg-red-100 dark:bg-red-950/50', border: 'border-red-300 dark:border-red-700' },
}
                    ...
                    {isRevealed && (
                      ...
                      >
                        <div className={`absolute left-2 w-5 h-5 rounded-full border-2 border-background flex items-center justify-center text-[8px] font-bold text-white ${
                          step.severity === 'critical' ? 'bg-red-600' : step.severity === 'high' ? 'bg-red-500' : step.severity === 'medium' ? 'bg-amber-500' : 'bg-emerald-500'
                        }`}>
                          {i + 1}
                        </div>
                  ...
                  {currentRiskData.map(d => {
                    ...
                    return (
                      <div key={d.dimension} className="text-center">
                        ...
                        <p className={`text-xs font-bold ${delta > 20 ? 'text-red-500' : delta > 10 ? 'text-amber-500' : 'text-emerald-500'}`}>
                          +{delta}
                        </p>
                      </div>
                    )
