...
export function SentimentDashboard({ companyName, onEntityClick, investigationData }: SentimentDashboardProps) {
  ...
  const healthColor = negativeRatio > 30 ? 'text-red-600 dark:text-red-400' : negativeRatio > 15 ? 'text-amber-600 dark:text-amber-400' : 'text-emerald-600 dark:text-emerald-400'
      ...
      {activeView === 'overview' && (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            {[
              { label: '正面舆情', value: overview.positive, icon: icons.TrendingUp, color: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-50 dark:bg-emerald-950/30' },
              { label: '中立舆情', value: overview.neutral, icon: icons.Minus, color: 'text-muted-foreground', bg: 'bg-muted' },
              { label: '负面舆情', value: overview.negative, icon: icons.TrendingDown, color: 'text-red-600 dark:text-red-400', bg: 'bg-red-50 dark:bg-red-950/30' },
