...
export function InvestigationProgress({
 ...
 {stage.status ==='done' ? (
 <icons.CheckCircle className="w-3.5 h-3.5 text-success shrink-0" />
 ) : stage.status ==='active' ? (
 <icons.Loader2 className="w-3.5 h-3.5 text-primary animate-spin shrink-0" />
 ) : stage.status ==='error' ? (
 <icons.XCircle className="w-3.5 h-3.5 text-destructive shrink-0" />
 ) : (
...
'text-muted-foreground'
 ...
 {agent.status ==='done' && (
 <p className="text-[10px] text-success">分析完成</p>
 )}
 </div>
 </motion.div>
 ))}
 ...
 researchProgress.confidence > 0.8 ?'bg-success' :
 researchProgress.confidence > 0.5 ?'bg-warning' :'bg-destructive'
 }`}
 initial={{ width: 0 }}
 ...
 speech.riskLevel ==='high' ?'bg-destructive/10 text-destructive' :
 speech.riskLevel ==='medium' ?'bg-warning/10 text-warning' :
'bg-success/10 text-success'
 ...
 initial={{ opacity: 0, y: 10 }}
 animate={{ opacity: 1, y: 0 }}
 className="bg-warning/10 border border-warning/20 rounded-xl p-4 space-y-2"
 >
 <div className="flex items-center gap-2">
