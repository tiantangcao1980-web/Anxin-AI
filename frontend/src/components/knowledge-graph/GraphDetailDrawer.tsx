...
import { heading, buttonStyle, cardStyle, iconSize, statusBadge, graphNodeColors } from'@/lib/design-tokens'
...
// 统一从 graphNodeColors 语义 token 取色 — 禁止在此处直接写 hex，参见 DESIGN.md §4
const TYPE_COLORS: Record<string, string> = {
'法规': graphNodeColors.law,
'案例': graphNodeColors.case,
'当事人': graphNodeColors.party,
'机构': graphNodeColors.organization,
'律师': graphNodeColors.lawyer,
'其他': graphNodeColors.other,
 law: graphNodeColors.law,
 entity: graphNodeColors.case,
 document: graphNodeColors.lawyer,
 query: graphNodeColors.query,
 conclusion: graphNodeColors.conclusion,
}
...
export function GraphDetailDrawer({ detail, loading, onClose, onClickRelation, onExpandInGraph, onEditEntity, onDeleteEntity, onInvestigate }: Props) {
 ...
 style={{
 background: `linear-gradient(135deg, ${TYPE_COLORS[detail.type] || graphNodeColors.other}15, ${TYPE_COLORS[detail.type] || graphNodeColors.other}05)`,
 border: `1px solid ${TYPE_COLORS[detail.type] || graphNodeColors.other}30`,
 }}
 >
 <div
 className="w-12 h-12 rounded-full mx-auto mb-2 flex items-center justify-center text-white font-medium text-lg"
 style={{ backgroundColor: TYPE_COLORS[detail.type] || graphNodeColors.other }}
 >
 {detail.name.charAt(0)}
 </div>
 <div className="font-medium text-sm text-foreground">{detail.name}</div>
 <span
 className="inline-block mt-1 text-[10px] px-2 py-0.5 rounded-full"
 style={{
 backgroundColor: (TYPE_COLORS[detail.type] || graphNodeColors.other) +'20',
 color: TYPE_COLORS[detail.type] || graphNodeColors.other,
 }}
 >
 ...
 <div>
 <h5 className="text-xs font-medium text-muted-foreground uppercase tracking-caption mb-2">属性</h5>
 ...
 <div>
 <h5 className="text-xs font-medium text-muted-foreground uppercase tracking-caption mb-2">
 ...
 {onDeleteEntity && (
 <button onClick={onDeleteEntity} className="flex-1 justify-center flex items-center gap-1 px-3 py-1.5 rounded-lg text-sm text-destructive hover:bg-destructive/20 transition-colors">
