...
export const LawyerAssistPanel = memo(function LawyerAssistPanel({
  ...
  const statusConfig: Record<string, { label: string; color: string; icon: React.ElementType }> = {
    online: { label: '在线', color: 'text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/30', icon: icons.Circle },
    busy: { label: '忙碌', color: 'text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/30', icon: icons.Clock },
  ...
  const commentTypeStyles: Record<string, { color: string; icon: React.ElementType; label: string }> = {
    ...
    approval: { color: 'text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/30 border-emerald-200 dark:border-emerald-800', icon: icons.CheckCircle, label: '通过' },
    revision: { color: 'text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/30 border-amber-200 dark:border-amber-800', icon: icons.Edit3, label: '修改' },
      ...
      <div className="h-full flex flex-col">
        ...
        <div className="px-4 py-3 bg-background border-b border-border flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-foreground">{activeAssistRequest.lawyerName}</p>
          ...
          <div className="flex items-center gap-2">
            ...
            {activeAssistRequest.status === 'pending' && (
              <span className="flex items-center gap-1 text-xs text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/30 px-2 py-1 rounded-full border border-amber-200 dark:border-amber-800">
            ...
            {isCompleted && (
              <span className="flex items-center gap-1 text-xs text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/30 px-2 py-1 rounded-full border border-emerald-200 dark:border-emerald-800">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
                  ...
                  {comment.type === 'revision' && (
                    <div className="flex gap-2 mt-3">
                      <button
                        ...
                        className="flex items-center gap-1 px-3 py-1.5 text-xs font-medium bg-background text-amber-700 dark:text-amber-300 rounded-lg border border-amber-300 dark:border-amber-700 hover:bg-amber-50 dark:hover:bg-amber-950/30 transition-colors"
                      >
            ...
            {isCompleted && comments.length > 0 && (
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="mt-4 p-4 bg-emerald-50 dark:bg-emerald-950/30 rounded-xl border border-emerald-200 dark:border-emerald-800"
              >
                <div className="flex items-center gap-2 mb-2">
                  <icons.CheckCircle className="w-5 h-5 text-emerald-600 dark:text-emerald-400" />
                  <span className="text-sm font-bold text-emerald-600 dark:text-emerald-400">审查完成</span>
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  <button
                    ...
                    className="flex items-center gap-1.5 px-4 py-2 text-xs font-medium bg-emerald-500 text-primary-foreground rounded-lg hover:bg-emerald-600 transition-colors shadow-sm"
                  >
                    <icons.CheckCircle className="w-3.5 h-3.5" /> 一键接受全部
                  </button>
                  <button
                    onClick={() => store.setDocumentOverlay('none')}
                    className="flex items-center gap-1.5 px-4 py-2 text-xs font-medium bg-background text-emerald-600 dark:text-emerald-400 rounded-lg border border-emerald-300 dark:border-emerald-700 hover:bg-emerald-50 dark:hover:bg-emerald-950/30 transition-colors"
                  >
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="px-4 py-3 bg-background border-b border-border">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-foreground">律师协助</span>
          </div>
      ...
      <div className="flex-1 overflow-y-auto">
              ...
              <div className="p-3 space-y-2 bg-muted/30">
                {assistHistory.slice(0, 5).map((req) => (
                  <div key={req.id} className="flex items-center gap-3 p-2.5 bg-background rounded-lg border border-border">
                    ...
                    <span className="text-[10px] px-1.5 py-0.5 bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400 rounded-full">已完成</span>
                  </div>
                ))}
              </div>
            </motion.div>
        ...
        {/* 请求类型选择 */}
        <div className="p-4">
          <p className="text-xs font-semibold text-muted-foreground mb-2 uppercase tracking-wide">协助类型</p>
          <div className="grid grid-cols-2 gap-2">
            {Object.entries(requestTypeLabels).map(([key, val]) => (
              ...
              >
                <p className={`text-xs font-bold ${requestType === key ? 'text-primary' : 'text-foreground'}`}>
                  {val.label}
                </p>
        ...
        {canvasContent && (
          <div className="mx-4 p-3 bg-muted/50 rounded-xl border border-border mb-4">
            ...
            <p className="text-sm font-semibold text-foreground truncate">{canvasContent.title || '未命名文档'}</p>
        ...
        <div className="px-4 mb-4">
          <p className="text-xs font-semibold text-muted-foreground mb-2 uppercase tracking-wide">备注说明（可选）</p>
          <textarea
            value={requestNote}
        ...
        <div className="px-4 pb-4">
          <p className="text-xs font-semibold text-muted-foreground mb-2 uppercase tracking-wide">选择律师</p>
                ...
                >
                  ...
                  <div className="relative">
                    ...
                    <div className={`absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 rounded-full border-2 border-background ${
                      lawyer.status === 'online' ? 'bg-emerald-500' : lawyer.status === 'busy' ? 'bg-amber-500' : 'bg-muted-foreground'
                    }`} />
                  </div>

                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-semibold text-foreground">{lawyer.name}</span>
