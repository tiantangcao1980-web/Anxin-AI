...
export function ReadinessIndicator({
  ...
  const colorMap = {
    high: { bar: 'bg-emerald-500', text: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-50 dark:bg-emerald-950/30', border: 'border-emerald-200 dark:border-emerald-800' },
    medium: { bar: 'bg-amber-500', text: 'text-amber-600 dark:text-amber-400', bg: 'bg-amber-50 dark:bg-amber-950/30', border: 'border-amber-200 dark:border-amber-800' },
    low: { bar: 'bg-red-500', text: 'text-red-600 dark:text-red-400', bg: 'bg-red-50 dark:bg-red-950/30', border: 'border-red-200 dark:border-red-800' },
  };
        ...
        <div className="mb-2">
          {filledSlots.map((slot) => (
            <div key={slot.key} className="flex items-center gap-1.5 py-0.5">
              <icons.CheckCircle className="w-3 h-3 text-emerald-500 flex-shrink-0" />
        ...
        <div className="mb-3">
          {missingElements.map((item, i) => (
            <div key={i} className="flex items-center gap-1.5 py-0.5">
              <icons.AlertCircle className="w-3 h-3 text-amber-500 flex-shrink-0" />
