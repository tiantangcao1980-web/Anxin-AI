...
const URGENCY_CONFIG = {
  ...
  urgent: { label: '紧急', color: 'text-amber-600 dark:text-amber-400 bg-amber-100 dark:bg-amber-900/30' },
        ...
        <div className="px-4 py-3 bg-background border-b border-border">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-foreground truncate">{activeWorkflow.title}</p>
          ...
          <div className="mt-3">
            ...
            <div className="w-full h-2 bg-muted rounded-full overflow-hidden">
              <motion.div
                className={`h-full rounded-full ${isAllDone ? 'bg-emerald-500' : 'bg-primary'}`}
                initial={{ width: 0 }}
        ...
        <div className="flex-1 overflow-y-auto p-4">
                ...
                <div key={s.id} className="flex gap-3">
                  ...
                  <div className="flex flex-col items-center">
                    ...
                    {idx < activeWorkflow.steps.length - 1 && (
                      <div className={`w-0.5 h-12 ${isCompleted ? 'bg-emerald-300' : 'border-border bg-border'}`} />
                    )}
                  </div>
                      ...
                      <div className="flex flex-wrap gap-2 mt-2">
                        ...
                        {s.assignee && (
                          <button
                            ...
                            className="flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/30 rounded-lg border border-amber-200 dark:border-amber-800 hover:bg-amber-100 dark:hover:bg-amber-900/30 transition-colors"
                          >
          ...
          {activeWorkflow.parties.length > 0 && (
            <div className="mt-4 p-4 bg-muted/50 rounded-xl border border-border">
              <p className="text-xs font-bold text-muted-foreground mb-3 uppercase tracking-wide">签署方</p>
                ...
                {activeWorkflow.parties.map((party) => (
                  <div key={party.id} className="flex items-center gap-3 p-2.5 bg-background rounded-lg border border-border">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${
                      party.status === 'signed' ? 'bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400' : 'bg-muted text-muted-foreground'
                    }`}>
                      {party.name[0]}
                    </div>
          ...
          {isAllDone && (
            <motion.div
              ...
              className="mt-4 p-6 bg-emerald-50 dark:bg-emerald-950/30 rounded-xl border border-emerald-200 dark:border-emerald-800 text-center"
            >
              <div className="w-16 h-16 mx-auto bg-emerald-100 dark:bg-emerald-900/30 rounded-full flex items-center justify-center mb-3">
                <icons.Award className="w-8 h-8 text-emerald-600 dark:text-emerald-400" />
              </div>
              <p className="text-base font-bold text-emerald-600 dark:text-emerald-400 mb-1">流程已全部完成</p>
              <p className="text-xs text-emerald-600 dark:text-emerald-400 mb-4">所有步骤已完成，文档已自动归档存证</p>
              <div className="flex justify-center gap-2">
                <button className="flex items-center gap-1.5 px-4 py-2 text-xs font-medium bg-emerald-500 text-primary-foreground rounded-lg hover:bg-emerald-600 transition-colors shadow-sm">
                  <icons.Download className="w-3.5 h-3.5" /> 下载签署件
                </button>
                <button className="flex items-center gap-1.5 px-4 py-2 text-xs font-medium bg-background text-emerald-600 dark:text-emerald-400 rounded-lg border border-emerald-300 dark:border-emerald-700 hover:bg-emerald-50 dark:hover:bg-emerald-950/30 transition-colors">
              ...
              >
                <div className="px-4 py-3 border-b border-border flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-bold text-foreground">电子签名</span>
                  </div>
              ...
              >
                <div className="px-4 py-3 border-b border-border flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-bold text-foreground">电子盖章</span>
                  </div>
                ...
                <div className="p-4">
                      ...
                      return (
                        ...
                        >
                          ...
                          <p className={`text-[10px] text-center font-medium ${
                            isActive ? 'text-red-700 dark:text-red-300' : 'text-muted-foreground'
                          }`}>
                            {sealName}
                          </p>
                          {isActive && (
                            <div className="absolute top-1.5 right-1.5">
                              <icons.CheckCircle className="w-4 h-4 text-red-500" />
                            </div>
                          )}
                        </button>
                      );
                  ...
                  {/* 安全验证提示 */}
                  <div className="p-3 bg-amber-50 dark:bg-amber-950/30 rounded-xl border border-amber-200 dark:border-amber-800 mb-2">
                    <div className="flex items-center gap-2 mb-1">
                      <icons.Lock className="w-3.5 h-3.5 text-amber-600 dark:text-amber-400" />
                      <span className="text-xs font-medium text-amber-700 dark:text-amber-300">安全验证</span>
                    </div>
                    <p className="text-[10px] text-amber-600 dark:text-amber-400">盖章操作将记录操作人身份、时间和IP地址，请确认文件内容无误后再操作。</p>
                  </div>
                </div>
    ...
    return (
      <div className="h-full flex flex-col">
        <div className="px-4 py-3 bg-background border-b border-border flex items-center gap-3">
          ...
          <span className="text-sm font-bold text-foreground">{config.label} — 配置</span>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-5">
          ...
          <div className="p-3 bg-muted/50 rounded-xl border border-border">
            ...
            <p className="text-sm font-bold text-foreground">{canvasContent?.title || '未命名文档'}</p>
          </div>

          {/* 盖章类型（仅盖章申请） */}
          {selectedDocType === 'seal_request' && config.sealTypes && (
            <div>
              <p className="text-xs font-bold text-muted-foreground mb-2 uppercase tracking-wide">印章类型</p>
              <div className="grid grid-cols-2 gap-2">
                {config.sealTypes.map((seal) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <icons.FileSignature className={`w-4 h-4 ${selectedSealType === seal ? 'text-red-600' : 'text-muted-foreground'}`} />
                      <span className={`text-xs font-medium ${selectedSealType === seal ? 'text-red-700 dark:text-red-300' : 'text-muted-foreground'}`}>
                        {seal}
                      </span>
                    </div>
                  </button>
          ...
          {/* 紧急程度 */}
          <div>
            <p className="text-xs font-bold text-muted-foreground mb-2 uppercase tracking-wide">紧急程度</p>
          ...
          {/* 添加签署方 */}
          <div>
            <p className="text-xs font-bold text-muted-foreground mb-2 uppercase tracking-wide">签署方/参与人</p>
          ...
          {/* 流程步骤预览 */}
          <div>
            <p className="text-xs font-bold text-muted-foreground mb-2 uppercase tracking-wide">流程步骤预览</p>
        ...
        <div className="p-4 border-t border-border bg-background">
          <button
            ...
            disabled={!isDocReady}
            className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-primary text-primary-foreground text-sm font-bold rounded-xl hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-sm active:scale-[0.98]"
          >
    ...
    return (
      <div className="h-full flex flex-col">
        <div className="px-4 py-3 bg-background border-b border-border flex items-center gap-3">
          ...
          <span className="text-sm font-bold text-foreground">选择业务类型</span>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          ...
          <div className={`p-3 rounded-xl border mb-4 ${
            ...
            <div className="flex items-center gap-2">
              ...
              <span className={`text-xs font-medium ${isDocReady ? 'text-emerald-600 dark:text-emerald-400' : 'text-amber-700 dark:text-amber-300'}`}>
                {isDocReady ? '文档已就绪，可发起签约/盖章流程' : '画布中暂无文档，请先生成或打开文档'}
              </span>
            </div>
            {isDocReady && detected && (
              <p className="text-[10px] text-emerald-600 dark:text-emerald-400 mt-1 ml-6">
      ...
      <div className="px-4 py-3 bg-background border-b border-border">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-foreground">签约盖章</span>
          </div>
          <button
            ...
            {signingWorkflows.map((wf) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-foreground truncate">{wf.title}</p>
                    <p className="text-xs text-muted-foreground">{config.label}</p>
                  ...
                  <div className="flex flex-col items-end gap-1">
                    {isAllDone ? (
                      <span className="text-[10px] px-1.5 py-0.5 bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400 rounded-full font-medium">已完成</span>
                    ) : (
