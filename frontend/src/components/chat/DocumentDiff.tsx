    ...
    >
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
            ...
            <div className="p-4 space-y-3">
              {originalContent.map((item) => (
                <div key={item.id}>
                  ...
                  {item.type === 'risk' && (
                    <div className="mt-1 flex items-start gap-2 text-xs text-red-600 dark:text-red-400 px-2">
          ...
          {/* Suggested */}
          <div className="bg-background rounded-xl border border-border overflow-hidden">
            <div className="bg-emerald-100 dark:bg-emerald-950/40 px-4 py-3 border-b border-emerald-200 dark:border-emerald-800">
              <div className="flex items-center gap-2">
                <icons.FileText className="w-4 h-4 text-emerald-700 dark:text-emerald-400" />
                <span className="font-medium text-sm text-emerald-900 dark:text-emerald-300">AI 优化版本</span>
              </div>
            </div>
            <div className="p-4 space-y-3">
              {suggestedContent.map((item, index) => (
                ...
                >
                  <p
                    className={`text-sm leading-relaxed p-2 rounded ${
                      item.type === 'added' || item.type === 'new'
                        ? 'bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800 text-emerald-900 dark:text-emerald-300'
                        : 'text-foreground'
                    }`}
                  >
                    {item.text}
                  ...
                  {(item.type === 'added' || item.type === 'new') && (
                    <div className="mt-1 flex items-start gap-2 text-xs text-emerald-700 dark:text-emerald-400 px-2">
        ...
        <div className="bg-background rounded-xl border border-border p-4">
          ...
          <div className="grid grid-cols-3 gap-4 text-center">
            <div className="p-3 bg-red-50 dark:bg-red-950/30 rounded-lg">
              ...
              <p className="text-xs text-red-700 dark:text-red-400 mt-1">风险点</p>
            </div>
            <div className="p-3 bg-emerald-50 dark:bg-emerald-950/30 rounded-lg">
              <p className="text-2xl font-bold text-emerald-600 dark:text-emerald-400">3</p>
              <p className="text-xs text-emerald-700 dark:text-emerald-400 mt-1">优化建议</p>
            </div>
