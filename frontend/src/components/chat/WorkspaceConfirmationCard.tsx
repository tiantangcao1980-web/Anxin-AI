...
export const WorkspaceConfirmationCard = memo(function WorkspaceConfirmationCard({
 ...
 {isConfirmed ? (
 <icons.CheckCircle2 className="w-4 h-4 text-success" />
 ) : (
 <icons.HelpCircle className="w-4 h-4 text-primary" />
 )}
 <span className="text-sm font-medium text-foreground">{confirmation.title}</span>
 </div>
 ...
 <icons.CheckCircle2 className="w-4.5 h-4.5 text-success" />
 ) : isSingle ? (
 isSelected ? (
