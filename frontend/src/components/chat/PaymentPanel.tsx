  ...
  return (
    <div className={`${cardStyle.base} ${className}`}>
      ...
      <div className="flex items-center gap-2 mb-4">
        <CreditCard className={`${iconSize.md} text-primary`} />
      ...
      {status === 'idle' && (
        <>
          <div className="mb-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <button
                ...
                className={`flex items-center justify-center gap-2 px-4 py-3 ${radius.button} border transition-all ${
                  method === 'wechat_pay'
                    ? 'border-[#07C160] bg-[#07C160]/5 text-[#07C160]'
                    : 'border-border bg-background text-muted-foreground hover:border-[#07C160]/40'
                }`}
              >
              ...
              <button
                ...
                className={`flex items-center justify-center gap-2 px-4 py-3 ${radius.button} border transition-all ${
                  method === 'alipay'
                    ...
                    : 'border-border bg-background text-muted-foreground hover:border-[#1677FF]/40'
                }`}
              >
                <svg viewBox="0 0 24 24" className={iconSize.md} fill="currentColor">
          ...
          <div className={`flex items-start gap-2 mb-4 p-3 ${statusColor.info} ${radius.badge}`}>
            <Shield className={`${iconSize.sm} mt-0.5 shrink-0`} />
      ...
      {status === 'pending' && (
        <div className="text-center py-4">
          <div className={`inline-flex flex-col items-center gap-3 p-6 ${cardStyle.flat} mb-4`}>
            {qrCode ? (
              <>
                <QrCode className="w-32 h-32 text-muted-foreground/30" />
              ...
              <>
                <Loader2 className={`${iconSize.xl} text-primary animate-spin`} />
          ...
          <div className="flex items-center justify-center gap-2 mb-4">
            <Clock className={`${iconSize.sm} text-amber-500`} />
            <span className={`${heading.micro} text-amber-600`}>
              订单将在 30 分钟后自动关闭
            </span>
          </div>
          ...
          >
            <RefreshCw className={`${iconSize.xs} mr-1 inline`} />
            模拟支付成功（开发模式）
          </button>
        </div>
      )}
